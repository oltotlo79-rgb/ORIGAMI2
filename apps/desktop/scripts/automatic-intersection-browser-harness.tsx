...
import { parsePathlessProjectSnapshot } from '../src/lib/recoveryClient.ts'
...
let scenario = 'balloon'; let revision = 7; let saved: ReturnType<typeof strictSnapshot> | null = null
Object.assign(window, { __ORIGAMI2_AUTOMATIC_INTERSECTION__: evidence, __TAURI_INTERNALS__: { invoke: async (command: string) => {
  if (command === 'add_edge') { evidence.addCalls += 1; if (scenario === 'duplicate') { evidence.duplicateRejects += 1; throw new Error('duplicate edge') } revision += 1; return strictSnapshot(scenario) }
  if (command === 'undo') { evidence.undoCalls += 1; revision += 1; return strictSnapshot('original') }
  if (command === 'redo') { evidence.redoCalls += 1; revision += 1; return strictSnapshot(scenario) }
...
function snapshot(kind: string) {
  ...
  return { project_instance_id: INSTANCE, project_id: PROJECT, name: '交差回帰', memo: '', current_path: null, revision, saved_revision: revision - 1, is_dirty: true,
    ...
    crease_pattern: { vertices, edges }, instruction_timeline: { steps: [] }, numeric_expressions: { undo_stack: [], redo_stack: [] }, geometric_constraints: { schema_version: 1, constraints: [] },
...
function strictSnapshot(kind: string) {
  const value = snapshot(kind)
  const admitted = parsePathlessProjectSnapshot(value)
  if (!admitted) {
    throw new Error('automatic intersection snapshot violated the current project wire schema')
  }
  return admitted
}
...
function Harness() {
  const [current, setCurrent] = useState(strictSnapshot('original')); const [notice, setNotice] = useState('ready')

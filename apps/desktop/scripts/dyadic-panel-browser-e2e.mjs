...
async function verifyAutomaticOppositePair(hinges, numerator, denominator, label) {
  const page = await openScenario('success', hinges)
  await page.getByLabel('Angle (degrees)').fill(String(2 * Math.atan2(numerator, denominator) * 180 / Math.PI))
...
try {
  ...
  await verifyAutomaticOppositePair(6, 2, 3, 'automatic C6 ratio 2/3')
  await verifyAutomaticOppositePair(8, 3, 7, 'automatic C8 ratio 3/7')
  await verifyAutomaticOppositePair(16, 63, 64, 'automatic C16 ratio 63/64')

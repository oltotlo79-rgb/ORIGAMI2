...
const squashSnapshot = JSON.parse(JSON.stringify({
  ...snapshot,
  name: 'つぶし折り',
  instruction_timeline: {
    steps: steps.map((step, index) => ({
      ...step,
      title: index === 0 ? 'つぶし折りの開始姿勢' : `つぶし折り ${index}`,
      description: index === 0
        ? step.description
        : step.description.replace('Miura atomic', 'つぶし折り'),
    })),
  },
})) as ProjectSnapshot
const petalSnapshot = JSON.parse(JSON.stringify({
  ...snapshot,
  name: '花弁折り',
  instruction_timeline: {
    steps: steps.map((step, index) => ({
      ...step,
      title: index === 0 ? '花弁折りの開始姿勢' : `花弁折り ${index}`,
      description: index === 0
        ? step.description
        : step.description.replace('Miura atomic', '花弁折り'),
    })),
  },
})) as ProjectSnapshot
const crimpSnapshot = JSON.parse(JSON.stringify({
  ...snapshot,
  name: '段折り',
  instruction_timeline: {
    steps: steps.map((step, index) => ({
      ...step,
      title: index === 0 ? '段折りの開始姿勢' : `段折り ${index}`,
      description: index === 0
        ? step.description
        : step.description.replace('Miura atomic', '段折り'),
    })),
  },
})) as ProjectSnapshot
const basicFoldSnapshot = (name: '山折り' | '谷折り'): ProjectSnapshot => JSON.parse(JSON.stringify({
  ...snapshot,
  name,
  instruction_timeline: {
    steps: steps.map((step, index) => ({
...
const valleySnapshot = basicFoldSnapshot('谷折り')
const withoutCertificate = (source: ProjectSnapshot): ProjectSnapshot => JSON.parse(JSON.stringify({
  ...source,
  instruction_timeline: {
    steps: source.instruction_timeline.steps.map((step, index) => ({
      ...step,
      description: index === 0
        ? step.description
        : `証明参照のない名前付き技法「${source.name}」の姿勢です。連続折り経路は未証明です。`,
      visual: index === 0 ? step.visual : { ...step.visual, path_certificate_reference_v1: undefined },
    })),
  },
})) as ProjectSnapshot
let exports = 0
...
let saveScenario: 'success' | 'cancel' | 'failure' = 'success'
...
Object.assign(window, { __TAURI_INTERNALS__: { invoke: async (command: string, args?: { format?: string }) => {
  ...
  if (command === 'save_instruction_export') {
    if (saveScenario === 'failure') throw new Error('atomic-save-failed')
    return { canceled: saveScenario === 'cancel' }
  }
...
function ExportLifecycle() { const [active, setActive] = useState(false); const [open, setOpen] = useState(false); const [event, setEvent] = useState('idle'); const [dialogFormat, setDialogFormat] = useState<'pdf' | 'svg_zip'>('pdf'); const [error, setError] = useState<string | null>(null); const [notice, setNotice] = useState<string | null>(null); const [successfulSaves, setSuccessfulSaves] = useState(0); const openSave = (nextFormat: 'pdf' | 'svg_zip', nextScenario: typeof saveScenario) => { ipc.length = 0; saveScenario = nextScenario; setDialogFormat(nextFormat); setError(null); setNotice(null); setActive(false); setOpen(true) }; const save = async (ack: boolean) => { try { const response = await saveInstructionExport('miura-export', snapshot.project_id, snapshot.revision, ack); if (response.canceled) { setNotice('保存先の選択をキャンセルしました。'); setEvent(`save-cancelled; successes=${successfulSaves}; cleanup=retained; format=${dialogFormat}; ipc=${ipc.join(',')}`); return } const next = successfulSaves + 1; setSuccessfulSaves(next); setOpen(false); setEvent(`saved; successes=${next}; cleanup=cleared; format=${dialogFormat}; ipc=${ipc.join(',')}`) } catch { setError('折り図を原子的に保存できませんでした。'); setEvent(`save-failed; successes=${successfulSaves}; cleanup=retained; format=${dialogFormat}; ipc=${ipc.join(',')}`) } }; return <><output data-testid="lifecycle">{event}</output><button onClick={() => { void (async () => { ipc.length = 0; saveScenario = 'success'; const generation = await beginInstructionExportGeneration(); setOpen(true); setActive(true); await getInstructionExportProgress(generation.export_id); setEvent(`progress; ipc=${ipc.join(',')}`) })() }}>Start progress lifecycle</button><button onClick={() => openSave('pdf', 'success')}>PDF save success</button><button onClick={() => openSave('svg_zip', 'success')}>SVG save success</button><button onClick={() => openSave('pdf', 'cancel')}>Save picker cancel</button><button onClick={() => openSave('svg_zip', 'failure')}>Atomic save failure</button>{open && <InstructionExportDialog format={dialogFormat} preview={{ ...preview, format: dialogFormat, suggested_file_name: dialogFormat === 'pdf' ? 'matrix.pdf' : 'matrix-svg.zip' }} busy={false} generationActive={active} phase={active ? 'building_document' : 'ready'} error={error} notice={notice} onFormatChange={setDialogFormat} onRetry={() => {}} onSave={(ack) => { void save(ack) }} onCancel={() => { void cancelInstructionExport('miura-export').then(() => { setActive(false); setOpen(false); setEvent(`cancelled; successes=${successfulSaves}; cleanup=cleared; ipc=${ipc.join(',')}`) }) }} />}</> }
function Harness() { const [technique, setTechnique] = useState<'miura' | 'reverse' | 'sink' | 'accordion' | 'layer-selective' | 'book-fold' | 'outside-reverse' | 'squash' | 'petal' | 'crimp' | 'mountain' | 'valley'>('miura'); const [uncertified, setUncertified] = useState(false); const certifiedSnapshot = technique === 'miura' ? snapshot : technique === 'reverse' ? reverseSnapshot : technique === 'sink' ? sinkSnapshot : technique === 'accordion' ? accordionSnapshot : technique === 'layer-selective' ? layerSelectiveSnapshot : technique === 'book-fold' ? bookFoldSnapshot : technique === 'outside-reverse' ? outsideReverseSnapshot : technique === 'squash' ? squashSnapshot : technique === 'petal' ? petalSnapshot : technique === 'crimp' ? crimpSnapshot : technique === 'mountain' ? mountainSnapshot : valleySnapshot; const activeSnapshot = uncertified ? withoutCertificate(certifiedSnapshot) : certifiedSnapshot; return <><output data-testid="exports">exports={exports}</output><button onClick={() => setTechnique('miura')}>Miura timeline</button><button onClick={() => setTechnique('reverse')}>Inside reverse timeline</button><button onClick={() => setTechnique('outside-reverse')}>Outside reverse timeline</button><button onClick={() => setTechnique('sink')}>Sink timeline</button><button onClick={() => setTechnique('accordion')}>Accordion timeline</button><button onClick={() => setTechnique('layer-selective')}>Layer selective timeline</button><button onClick={() => setTechnique('book-fold')}>Book fold timeline</button><button onClick={() => setTechnique('squash')}>Squash fold timeline</button><button onClick={() => setTechnique('petal')}>Petal fold timeline</button><button onClick={() => setTechnique('crimp')}>Crimp fold timeline</button><button onClick={() => setTechnique('mountain')}>Mountain fold timeline</button><button onClick={() => setTechnique('valley')}>Valley fold timeline</button><button onClick={() => setUncertified(true)}>Uncertified timelines</button><button onClick={() => { format = 'pdf'; scenario = 'valid' }}>PDF mode</button><button onClick={() => { format = 'svg_zip'; scenario = 'valid' }}>SVG mode</button><button onClick={() => { scenario = 'stale' }}>Stale revision</button><button onClick={() => { scenario = 'tamper' }}>Tamper DTO hash</button><InstructionTimelinePanel snapshot={activeSnapshot} appliedPose={null} poseModelKey={`${technique}-${uncertified}`} manualPoseChangeSequence={0} coreBusy={false} benchmarkActive={false} fileOperationActive={false} exportAvailable exportButtonRef={{ current: null }} animationExportButtonRef={{ current: null }} runNativeEdit={async () => true} applyStepPose={() => true} onExport={() => { void exportThroughProductionIpc() }} onAnimationExport={() => {}} /><ExportLifecycle /></> }

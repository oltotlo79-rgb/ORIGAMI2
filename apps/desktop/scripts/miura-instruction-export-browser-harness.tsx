...
function Harness() { const [technique, setTechnique] = useState<'miura' | 'reverse' | 'sink' | 'accordion' | 'layer-selective' | 'book-fold' | 'outside-reverse' | 'squash' | 'petal' | 'crimp' | 'mountain' | 'valley'>('miura'); const activeSnapshot = technique === 'miura' ? snapshot : technique === 'reverse' ? reverseSnapshot : technique === 'sink' ? sinkSnapshot : technique === 'accordion' ? accordionSnapshot : technique === 'layer-selective' ? layerSelectiveSnapshot : technique === 'book-fold' ? bookFoldSnapshot : technique === 'outside-reverse' ? outsideReverseSnapshot : technique === 'squash' ? squashSnapshot : technique === 'petal' ? petalSnapshot : technique === 'crimp' ? crimpSnapshot : technique === 'mountain' ? mountainSnapshot : valleySnapshot; return <><output data-testid="exports">exports={exports}</output><button onClick={() => setTechnique('reverse')}>Inside reverse timeline</button><button onClick={() => setTechnique('outside-reverse')}>Outside reverse timeline</button><button onClick={() => setTechnique('sink')}>Sink timeline</button><button onClick={() => setTechnique('accordion')}>Accordion timeline</button><button onClick={() => setTechnique('layer-selective')}>Layer selective timeline</button><button onClick={() => setTechnique('book-fold')}>Book fold timeline</button><button onClick={() => setTechnique('squash')}>Squash fold timeline</button><button onClick={() => setTechnique('petal')}>Petal fold timeline</button><button onClick={() => setTechnique('crimp')}>Crimp fold timeline</button><button onClick={() => setTechnique('mountain')}>Mountain fold timeline</button><button onClick={() => setTechnique('valley')}>Valley fold timeline</button><button onClick={() => { format = 'pdf'; scenario = 'valid' }}>PDF mode</button><button onClick={() => { format = 'svg_zip'; scenario = 'valid' }}>SVG mode</button><button onClick={() => { scenario = 'stale' }}>Stale revision</button><button onClick={() => { scenario = 'tamper' }}>Tamper DTO hash</button><InstructionTimelinePanel snapshot={activeSnapshot} appliedPose={null} poseModelKey={technique} manualPoseChangeSequence={0} coreBusy={false} benchmarkActive={false} fileOperationActive={false} exportAvailable exportButtonRef={{ current: null }} animationExportButtonRef={{ current: null }} runNativeEdit={async () => true} applyStepPose={() => true} onExport={() => { void exportThroughProductionIpc() }} onAnimationExport={() => {}} /><ExportLifecycle /></> }

...
} from '../lib/protrusionDimensionEditorText.ts'
  ...
  return <li>
    ...
    <label>{label(TEXT.rootWidthLabel)}
      <input type="number" min="0.1" max="1000" step="0.1"
        aria-label={bindingLabel(TEXT.ariaBindingMillimetres, TEXT.rootWidth)}
    ...
    <label>{label(TEXT.tipWidthLabel)}
      <input type="number" min="0.1" max="1000" step="0.1"
        aria-label={bindingLabel(TEXT.ariaBindingMillimetres, TEXT.tipWidth)}
    ...
    <label>{label(TEXT.lengthLabel)}
      <input type="number" min="0.1" max="100000" step="0.1"
        aria-label={bindingLabel(TEXT.ariaBindingMillimetres, TEXT.length)}
        value={target.length_tenths_mm / 10}
        onChange={(event) => update('length_tenths_mm', Number(event.currentTarget.value))} />
    </label>
    <label>{label(target.symmetry === 'bilateral'
      ? TEXT.bilateralSpacingLabel : TEXT.thicknessLabel)}
      <input type="number" min="0.1" max="1000" step="0.1"
        aria-label={bindingLabel(TEXT.ariaBindingMillimetres, target.symmetry === 'bilateral'
          ? TEXT.bilateralSpacing : TEXT.thickness)}
        value={target.thickness_tenths_mm / 10}
        onChange={(event) => update('thickness_tenths_mm', Number(event.currentTarget.value))} />
    </label>
    <label>{label(TEXT.mountVerticalLabel)}
      <input type="number" min="-10000" max="10000" step="0.1"
        aria-label={bindingLabel(TEXT.ariaBindingMillimetres, TEXT.mountVertical)}
        value={target.position_tenths_mm[1] / 10}
        onChange={(event) => updatePosition(1, Number(event.currentTarget.value))} />
    </label>
    <label>{label(TEXT.mountForeAftLabel)}
      <input type="number" min="-10000" max="10000" step="0.1"
        aria-label={bindingLabel(TEXT.ariaBindingMillimetres, TEXT.mountForeAft)}
        value={target.position_tenths_mm[2] / 10}
        onChange={(event) => updatePosition(2, Number(event.currentTarget.value))} />
    </label>
    <label>{label(TEXT.directionHorizontal)}
      <input type="number" min="-1" max="1" step="0.001"
        aria-label={bindingLabel(TEXT.ariaBinding, TEXT.directionHorizontal)}
        value={target.direction_milli[0] / 1_000}
        onChange={(event) => updateDirection(0, Number(event.currentTarget.value))} />
    </label>
    <label>{label(TEXT.directionVertical)}
      <input type="number" min="-1" max="1" step="0.001"
        aria-label={bindingLabel(TEXT.ariaBinding, TEXT.directionVertical)}
        value={target.direction_milli[1] / 1_000}
        onChange={(event) => updateDirection(1, Number(event.currentTarget.value))} />
    </label>
    <label>{label(TEXT.curvatureLabel)}<input type="number" min="-360" max="360" step="1"
      aria-label={bindingLabel(TEXT.ariaBindingDegrees, TEXT.curvature)} value={target.curvature_degrees}
    ...
    <label>{label(TEXT.motionMinimumLabel)}<input type="number" min="-360" max="360" step="1"
      aria-label={bindingLabel(TEXT.ariaBindingDegrees, TEXT.motionMinimum)} value={target.motion_degrees[0]}
      onChange={(event) => updateMotion(0, Number(event.currentTarget.value))} /></label>
    <label>{label(TEXT.motionMaximumLabel)}<input type="number" min="-360" max="360" step="1"
      aria-label={bindingLabel(TEXT.ariaBindingDegrees, TEXT.motionMaximum)} value={target.motion_degrees[1]}
      onChange={(event) => updateMotion(1, Number(event.currentTarget.value))} /></label>
    <label>{label(TEXT.joint)}<select aria-label={bindingLabel(TEXT.ariaBinding, TEXT.joint)} value={target.joint}
      onChange={(event) => onChange({ ...target, joint: event.currentTarget.value as Protrusion['joint'] })}>
      <option value="fixed">{label(TEXT.jointFixed)}</option><option value="hinge">{label(TEXT.jointHinge)}</option><option value="ball">{label(TEXT.jointBall)}</option>
    </select></label>
    <label>{label(TEXT.side)}<select aria-label={bindingLabel(TEXT.ariaBinding, TEXT.side)} value={target.side}
      onChange={(event) => onChange({ ...target, side: event.currentTarget.value as Protrusion['side'] })}>
      <option value="front">{label(TEXT.sideFront)}</option><option value="back">{label(TEXT.sideBack)}</option><option value="either">{label(TEXT.sideEither)}</option>
    </select></label>
    <label>{label(TEXT.priority)}<input type="number" min="1" max="100" step="1" aria-label={bindingLabel(TEXT.ariaBinding, TEXT.priority)}
    ...
    <button type="button" disabled={!canRemove} onClick={onRemove}>{label(TEXT.remove)}</button>

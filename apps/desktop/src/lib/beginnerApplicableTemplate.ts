import type { BeginnerCandidateResponseV1 } from './coreClient.ts'

type GeneratedPlanKind =
  BeginnerCandidateResponseV1['generated_plans'][number]['kind']

const APPLICABLE_TEMPLATE_KINDS = new Set<GeneratedPlanKind>([
...
export type BeginnerApplicableTemplate = Exclude<
  GeneratedPlanKind,
  'vertical_book_fold' | 'horizontal_book_fold' | 'diagonal_fold'
>

export function isBeginnerApplicableTemplate(
  kind: GeneratedPlanKind,
): kind is BeginnerApplicableTemplate {
  return APPLICABLE_TEMPLATE_KINDS.has(kind)
}

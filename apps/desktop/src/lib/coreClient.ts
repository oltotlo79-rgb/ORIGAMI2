...
function normalizeBeginnerCandidateResponse(
  ...
  const generatedPlans = response.generated_plans.map((plan) => {
    ...
    if (
      !record
      || record.schema_version !== 1
      || !['symmetric_four_leg_base', 'symmetric_wing_base', 'symmetric_bird_base', 'symmetric_fish_base', 'symmetric_ear_base', 'symmetric_horn_base', 'symmetric_antenna_base', 'symmetric_insect_leg_pair_base', 'symmetric_six_leg_base', 'center_axis_tail_base', 'center_axis_horn_base', 'center_axis_antenna_base', 'vertical_book_fold', 'horizontal_book_fold', 'diagonal_fold'].includes(String(record.kind))
      || !pattern
      || !Array.isArray(pattern.vertices)
      || pattern.vertices.length < 2
      || pattern.vertices.length > (record.kind === 'symmetric_six_leg_base' ? 13 : 5)
      || !Array.isArray(pattern.edges)
      || pattern.edges.length < 1
      || pattern.edges.length > (record.kind === 'symmetric_six_leg_base' ? 12 : 4)
      || !Array.isArray(record.instruction_codes)
      || record.instruction_codes.length !== 1
      || !record.instruction_codes.every((code) =>
        ['symmetric_four_leg_base', 'symmetric_wing_base', 'symmetric_bird_base', 'symmetric_fish_base', 'symmetric_ear_base', 'symmetric_horn_base', 'symmetric_antenna_base', 'symmetric_insect_leg_pair_base', 'symmetric_six_leg_base', 'center_axis_tail_base', 'center_axis_horn_base', 'center_axis_antenna_base', 'book_fold_vertical', 'book_fold_horizontal', 'diagonal_fold'].includes(String(code)))
    ) return null
...
export async function suggestBeginnerReferenceModelFeatures(
  ...
  if (!constraints || ![1, 3].includes(protrusions.length)
    ...
    || suggestion.pair_bindings.length !== (['tail', 'horn', 'antenna'].includes(String(suggestion.suggested_part_kind)) && protrusions[0]?.count === 1 ? 0 : protrusions.length)

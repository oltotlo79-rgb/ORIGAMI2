...
function normalizeBeginnerCandidateResponse(
  ...
  const generatedPlans = response.generated_plans.map((plan) => {
    ...
    if (
      ...
      || pattern.vertices.length > (record.kind === 'symmetric_six_leg_base' ? 13 : record.kind === 'composite_tail_ear_base' ? 6 : 5)
      || !Array.isArray(pattern.edges)
      || pattern.edges.length < 1
      || pattern.edges.length > (record.kind === 'symmetric_six_leg_base' ? 12 : record.kind === 'composite_tail_ear_base' ? 5 : 4)

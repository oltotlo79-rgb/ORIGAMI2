...
export const GEOMETRIC_CONSTRAINT_PANEL_TEXT = Object.freeze({
  ...
  semanticMusCertified: text(
    '決定論的binary64意味論的最小コアを認証しました（{count}件、直接オラクル{calls}回、削除証人{checks}件、作業量{work}）。証人方式：現在配置{current}件、軸厳密化{axis}件、単一制約構成{constructive}件、二制約構成{pairConstructive}件、二制約代数縮退{pairAlgebraic}件、有界長さ制約構成{lengthConstructive}件、ゼロ長閉包構成{zeroClosure}件、アンカー鏡映残差限定{mirrorResidual}件、単位長・平行・固定角残差証人{unitParallelFixedAngleResidual}件、単位終端2段平行・直角残差証人{unitTerminalTwoHopParallelAngleResidual}件、単位長2段平行・残差証人{unitTwoHopParallelResidual}件。{scope} コア：{ids}',
    'Certified a deterministic-binary64 semantic minimal core ({count} constraints, {calls} direct-oracle calls, {checks} deletion witnesses, work {work}). Witness methods: {current} current-assignment, {axis} axis-exactification, {constructive} single-constraint constructive, {pairConstructive} pair-constraint constructive, {pairAlgebraic} pair-constraint algebraic-collapse, {lengthConstructive} bounded length-only constructive, {zeroClosure} bounded zero-length-closure constructive, {mirrorResidual} anchored-mirror residual-only, {unitParallelFixedAngleResidual} unit parallel-fixed-angle residual witnesses, {unitTerminalTwoHopParallelAngleResidual} unit-terminal two-hop parallel-angle residual witnesses, {unitTwoHopParallelResidual} unit two-hop parallel residual witnesses. {scope} Core: {ids}',
  ),

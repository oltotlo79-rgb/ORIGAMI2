...
pub(super) fn unit_parallel_fixed_angle_inventory_fixture() -> SemanticFixture {
    unit_parallel_fixed_angle_fixture(45.0)
}

fn unit_parallel_fixed_angle_fixture(angle_degrees: f64) -> SemanticFixture {
...
fn deletion_document(
    ...
    deletion_document_from_records(&fixture.records, removed)
}

fn deletion_document_from_records(
...
fn exact_three_id_core_recertifies_every_deletion_in_the_dedicated_phase() {
    for angle_degrees in [45.0, 135.0] {
        let fixture = unit_parallel_fixed_angle_fixture(angle_degrees);
        ...
        for removed in &fixture.records {
            assert!(
                ...
                .is_some(),
                "each exact {angle_degrees}-degree deletion must be independently recertified",
            );
        }
...
fn witness_bits_pin_unit_diagonal_and_finite_max_over_infinity_boundaries() {
    ...
    let supplementary_diagonal_angle =
        deterministic_atan2_v1(diagonal, -diagonal).expect("supplementary unit diagonal atan2");
    assert_eq!(
        supplementary_diagonal_angle.to_bits(),
        0x4002_d97c_7f33_21d2
    );
    assert_eq!(
        deterministic_fixed_angle_residual_binary64_v1(supplementary_diagonal_angle, 135.0,)
            .to_bits(),
        0,
    );
    let supplementary_overflow_angle =
        deterministic_atan2_v1(square, -square).expect("supplementary finite equal atan2 operands");
    assert_eq!(
        supplementary_overflow_angle.to_bits(),
        supplementary_diagonal_angle.to_bits(),
    );
    assert_eq!(
        deterministic_fixed_angle_residual_binary64_v1(supplementary_overflow_angle, 135.0,)
            .to_bits(),
        0,
    );
}
        ...
        for fixed_role in [0, 1] {
            for direction_mask in 0_u8..4 {
                for reverse_operands in [false, true] {
                    let mut fixture = unit_parallel_fixed_angle_fixture(angle_degrees);
                    ...
                    *first_edge = first;
                    *second_edge = second;
                    assert_eq!(*stored_angle, angle_degrees);
                    assert!(
...
fn constructor_rejects_nonexact_values_documents_and_nonstar_topology() {
    for exact_angle in [45.0_f64, 135.0_f64] {
        let fixture = unit_parallel_fixed_angle_fixture(exact_angle);
        let removed = fixture.records[0].id;
        let deletion = deletion_document(&fixture, removed);

        for angle in [exact_angle.next_down(), exact_angle.next_up(), 90.0] {
            ...
            *angle_degrees = angle;
            let changed_deletion = deletion_document_from_records(&changed, removed);
            assert!(
                construct_unit_parallel_fixed_angle_residual_exact_deletion_assignment_v1(
                    &fixture.pattern,
                    &changed,

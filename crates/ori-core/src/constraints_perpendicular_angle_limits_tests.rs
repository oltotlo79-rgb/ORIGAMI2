...
use super::perpendicular_angle_tests::{assert_target, core_records};
use super::same_orientation_angle_tests::{Fixture, document, record, sorted_ids};
use super::*;
use crate::{
...
fn direct_proof_and_bounded_oracle_cover_four_eight_sixteen_and_seventeen_records() {
    for count in [4, 8, 16, 17] {
        ...
        let mut records = core_records(&fixture, false, 45.0);
        ...
        assert_target(&prepared.preflight(), &fixture, false, &expected_ids);
        if count == 17 {
            assert_eq!(
...
fn exact_resource_and_work_limits_are_admitted_and_one_short_is_fail_closed() {
    ...
    let prepared = prepare_geometric_constraints_v1(&fixture.pattern, &raw, exact)
        .expect("exact storage/work limits must prepare");
    assert_target(&prepared.preflight(), &fixture, false, &ids);
...
fn solver_and_verifier_cannot_bypass_the_promoted_preflight() {
    let fixture = Fixture::new();
    let raw = document(core_records(&fixture, false, 45.0));
    assert_eq!(

...
pub struct PreparedStackedFoldRequestedPoseV1 {
    ...
    request_instance_anchor: Arc<()>,
}

/// Process-local identity retained by a speculative Apply ticket for one
/// exact prepared request.
///
/// The request anchor rejects independently re-prepared ABA values, while the
/// model and both pose clones independently retain the native issuer and
/// solve-instance identities. This value never crosses a persistence or wire
/// boundary.
#[derive(Debug)]
pub(crate) struct PreparedStackedFoldRequestIssuerSealV1 {
    request_instance_anchor: Arc<()>,
    model: MaterialTreeKinematicsModel,
...
impl PreparedStackedFoldRequestIssuerSealV1 {
    pub(crate) fn capture(requested: &PreparedStackedFoldRequestedPoseV1) -> Self {
        let initial = requested.initial();
        Self {
            request_instance_anchor: Arc::clone(&requested.request_instance_anchor),
            model: initial.target().model().clone(),
            source_pose: initial.pose().clone(),
            target_pose: requested.pose().clone(),
        }
    }

    /// Rebinds every process-local issuer dimension before any semantic
    /// metadata is consulted.
    ///
    /// `MaterialTreeKinematicsModel` does not expose its private Arc directly;
    /// mutual ownership of the retained and live pose instances is therefore
    /// the exact model-instance comparison. The two `same_instance` checks
    /// additionally reject repeated solves with identical angle bits.
    pub(crate) fn authenticates(&self, requested: &PreparedStackedFoldRequestedPoseV1) -> bool {
        let initial = requested.initial();
        let live_model = initial.target().model();
        Arc::ptr_eq(
            &self.request_instance_anchor,
            &requested.request_instance_anchor,
        ) && self.model.owns_pose(&self.source_pose)
            && self.model.owns_pose(&self.target_pose)
            && self.model.owns_pose(initial.pose())
            && self.model.owns_pose(requested.pose())
            && live_model.owns_pose(&self.source_pose)
            && live_model.owns_pose(&self.target_pose)
            && self.source_pose.same_instance(initial.pose())
            && self.target_pose.same_instance(requested.pose())
    }
}
...
pub fn prepare_stacked_fold_requested_pose_v1(
    ...
    Ok(PreparedStackedFoldRequestedPoseV1 {
        ...
        pose,
        requested_angle_degrees,
        request_instance_anchor: Arc::new(()),
    })
}
...
pub fn revalidate_current_graph_non_flat_layer_order_v1(
    ...
    let geometry = MaterialHingeGraphGeometry::prepare(pattern, paper, &topology, limits)?;
    ...
    if pose.hinge_angles().as_slice().iter().all(|angle| {
        angle.angle_degrees().to_bits() == 0.0_f64.to_bits()
            || angle.angle_degrees().to_bits() == 180.0_f64.to_bits()
    }) {
        return Err(PrepareStackedFoldNonFlatLayerOrderErrorV1::NotNonFlatEndpoint);
    ...
    let planes = material_faces
        .iter()
        .map(|face| target_graph_face_plane(&geometry, &pose, face.face_id))
        .collect::<Result<Vec<_>, _>>()?;
    let folded_faces = material_faces
        .iter()
        .zip(&planes)
    ...
    for first in 0..planes.len() {
        for second in first + 1..planes.len() {
            let cross = point_length(point_cross(
                point_values(planes[first].1),
                point_values(planes[second].1),
            ));
            if !cross.is_finite() {
    ...
    let fresh = StackedFoldNonFlatLayerOrderV1 {
        identity_namespace,
        target_revision: revision,
        target_fingerprint: fingerprint,
    ...
    if expected_archive.is_some_and(|expected| *expected != fresh) {
...
/// The graph pose is re-solved by its owning geometry so a foreign or stale
/// closure observation cannot be substituted by matching identifiers.
pub fn prepare_stacked_fold_graph_non_flat_layer_order_v1(
...
fn prepare_stacked_fold_graph_non_flat_layer_order_from_source_v1(
    ...
    let replay = target.hinge_geometry.solve_closed(
        &target.audit,
        pose.fixed_face(),
        pose.hinge_angles(),
        STACKED_FOLD_GRAPH_CLOSURE_TOLERANCE_V1,
    ...
    if ids != target.hinge_geometry.face_ids() {
        return Err(PrepareStackedFoldNonFlatLayerOrderErrorV1::TargetPoseMismatch);
    }
    let tested_face_pairs = material_faces
        .len()
    ...
    let points = boundary
        ...
        .map(|vertex| {
            transform
                ...
                .map_err(PrepareStackedFoldNonFlatLayerOrderErrorV1::from)
        })
        .collect::<Result<Vec<_>, _>>()?;
    face_plane_from_points(points)
}

fn face_plane_from_points(
    points: Vec<Point3>,
) -> Result<(Point3, Point3, Vec<Point3>), PrepareStackedFoldNonFlatLayerOrderErrorV1> {
    let origin = *points
        .first()
    ...
    for index in 1..points.len().saturating_sub(1) {
        let cross = point_cross(
            point_delta(points[index], origin),
            point_delta(points[index + 1], origin),
        );

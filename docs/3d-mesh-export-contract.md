...
本書は、ORIGAMI2の認証済み現在3D姿勢をOBJ、binary STL、glTF 2.0 GLBへ書き出す製品経路と、その純粋Rust形式基盤を定める。形式基盤は1個の静的indexed triangle meshを検証し、同じ検証済みmeshから3形式のbytesを決定論的に生成して、独立verifierで再検査する。

製品経路は、現在表示中の認証済みapplied poseから紙の中央面meshを決定論的に構築し、project instance、project ID、revision、geometry fingerprint、pose generation、紙厚bit列へ束縛する。生成bytesはnativeのimmutable stageだけに保持し、UIにはbounded metadataと固定warningだけを返す。利用者が情報損失を明示確認した後にnative保存dialogを開き、保存直前にも同じ束縛を再検証して原子的に保存する。これにより要件IO-007の静的3形式書き出し経路を実装する。

要件IO-008は、Blender、3D printer、Web・他3Dアプリへ静的fileを受け渡せる段階までを部分実装とする。外部readerによる実機受入、animation、material・texture、厚み付きclosed solidおよび印刷可能性保証は、IO-008とQAの残課題である。
...
- 全形式で、中央面のみ、厚み付きsolidなし、material・texture・animationなし、project意味情報なしを表示する。STLでは、頂点index・頂点normalが失われtriangle soup・facet normalとなることと、3D printabilityを保証しないことを追加表示する。
...
- STLを対象slicerへ読み込み、mm scale、向き、triangle数を確認する。中央面はzero-thicknessであり、印刷可能なclosed solidになることを期待値にしない。
...
- Blender向けmaterial・紙色・表裏、texture・UV、編集workflowを実装・受け入れる。
- 3D printer向けに中央面を厚み付きclosed solidへ変換し、穴閉鎖、manifold、watertight、self-intersection、overhang、minimum wall thicknessなどを判定する。初版の静的STLはこれらを保証しない。

...
本書は、ORIGAMI2の認証済み現在3D姿勢をOBJ、binary STL、glTF 2.0 GLBへ書き出す製品経路と、その純粋Rust形式基盤を定める。形式基盤は1個の静的indexed triangle meshを検証し、同じ検証済みmeshから3形式のbytesを決定論的に生成して、独立verifierで再検査する。GLBはPBR materialを内包し、紙厚付き形状では正規化RGBA頂点色で表裏色とedge色を保持する。
...
要件IO-008は、Blender、3D printer、Web・他3Dアプリへ静的fileを受け渡せる段階、GLB表裏色、および面別closed solidまでを部分実装とする。外部readerによる実機受入、animation、texture、面間unionおよび印刷可能性保証は、IO-008とQAの残課題である。
...
- 紙厚0では中央面のみ、紙厚正では面別closed solidであることを表示する。GLBの紙色と紙厚付き形状の表裏色、未対応のtexture・animation、project意味情報の欠落を表示する。STLでは、頂点index・頂点normalが失われtriangle soup・facet normalとなることと、面別solidの重なり・隙間を含め3D printabilityを保証しないことを追加表示する。
...
形式内verifierと自動回帰は、構文、単位、軸、index、winding、数値範囲、決定性、stage束縛および情報損失確認を検証する。さらに独立したecosystem readerとして、`tobj 4.0.4`、`stl_io 0.11.0`、`gltf 1.4.1`へ同じ生成bytesを渡し、geometry、index、頂点色を再読込する自動受入を行う。一方、第三者GUIアプリのversion差、表示、編集、slicingの解釈まで自動readerだけで保証してはならない。次はIO-008または配布候補QAとして別に受け入れる。
...
- Blender向けtexture・UV、編集workflowを実装・受け入れる。

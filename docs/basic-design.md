...
- 単位は内部基準へ正規化し、表示時に変換する。mm、cm、inch、紙辺比の保存・換算・
  参照失効・入力精度の契約は[長さ表示単位の契約](length-display-units.md)を正本とする。
...
`topology_contact_policy_v1`の4×10・全40セルは互換基準として凍結する。正厚の正面積境界接触を独立させた`topology_contact_policy_v2`の4×11・全44セルをnative証拠生成器の正本とし、frontendとnative `ori-collision`が同じ正規JSON corpusへ回帰する。純粋表と実姿勢からの証拠到達性は[衝突分類v2実装カバレッジ台帳](collision-classification-coverage-v2.md)で分離して追跡する。これら純粋表の公開enumまたは戻り値は認証済み証拠ではなく、native実行系はcurrent poseとtriangle pairへ結合したprivate certificate/capabilityなしに衝突除外・ヒンジ許容・project mutationへ使用してはならない。runtimeで`same_face`がpair dispatcherへ到達した場合は表の`ignored_self`を採用せず、内部不整合として`indeterminate`へ閉じる。

native静的判定の最初の下位実装は、単一material face・no-hingeのunordered pair数0だけをopaqueなgeometry proofとして肯定する。exact model issuer、pose instance、紙厚bits、`centered_mid_surface_v1`、policy/model/proof IDへ結合するが、project、revision、current pose certificate identity、pose generationは証明しないため、単独ではcurrent collision certificateまたはmutation authorityではない。desktopはこれを同一`Arc`のcurrent pose certificateとgenerationへprivateに再結合し、発行時と観察時にproject/revision/fingerprint/model/pose/紙厚/proof identityを再検証する限定Cを実装した。観察callbackのpanic後もproject/pose mutexをpoisonせず、mutation/SIM-010 authorityは付与しない。複数面は全pairの幾何証拠が揃うまでblocking errorとし、0件解析を安全完了へ読み替えない。

厚さ0のnative下位classifierは、有限binary64の三角形頂点を`2^-1074`単位の整数へexact変換し、離間・点接触・境界線接触・共面正面積・横断交差・判定保留を頂点順と面交換に依存せず分類する。全faceの単純境界検証、凹多角形と同方向collinear頂点を含む決定論的exact ear clipping、面積・外周/内部辺incidence、全face/triangle-pair coverage、作業上限は完成した。各面のworld頂点は同一face transformのexact affine像として構成し、個別丸めで共面性を壊さない。認証済み共有点または完全共有辺上だけの接触は貫通へ昇格させず、共有要素外へ広がる共面正面積または直接証明できる横断は貫通証拠とする。共有頂点許容はexact singletonに加えて、検証済み変換local `+Y`材料法線の内積がbinary64境界`1e-10`より厳密に大きい場合だけ発行する。人工三角形分割対角線上の低次元交差は、両material faceの交線上coverageと実外周endpointで作るexact event cellへ再構成し、両relative interiorが共通する正長区間だけを横断へ昇格する。source共有関係が正しくcurrent座標だけが不一致のpairは全triangleを走査してcoverageを保持するが、任意小の残差が偽横断を作る反例があるためraw証拠を採用せず明示的`indeterminate`へ閉じる。材料境界のみ、凹形状の空隙、退化、割当失敗、証明不足も非貫通または判定保留とする。watertight共有ヒンジ、有限ヒンジモデルおよび正厚の完全な証拠集合が完成するまで、複数面のproduction geometry proofはblockingを維持する。

有限な正厚では、`centered_mid_surface_v1`のclosed solidが自身の材料中央面を内部に含むことを、限定的な貫通の十分条件へ使う。同じissuer-bound poseについて、canonical exact `E`のzero-width三角形と、保存binary64 affine係数・rest頂点をactual-mm有理数へ直接liftした`F`のzero-width三角形が、ともに同じcanonical face pairのstrict transversalを証明した場合だけ、正厚材料の貫通を専用`proven_positive_thickness_penetration`としてblocking肯定する。strict transversalは両三角形のrelative interiorへ共有要素外の正長交差が広がることを必須とし、共有頂点・共有辺だけの点/線接触、共面重なり、片側だけの肯定、独立envelope box、三角柱SATの不確定、共有ヒンジまたは非三角面から発行しない。この十分条件は正厚solidの安全証明、境界面接触、共面重なり、正体積三角柱SAT、有限ヒンジcorridorまたは層ずらしを完成させるものではない。肯定できないpairは従来どおり安全側の判定保留とし、multi-faceのpublic geometry proof成功集合を広げない。

watertight姿勢の局所回転核として、非公開の`rational_cayley_local_rotation_v1`を`ori-collision`へ実装した。finite binary64の軸端点と角度量をbit-exactな有理数へ変換し、回転符号を独立した`+1 / -1`として適用する。0度と180度は厳密な専用枝、一般角はMachin π・外向きTaylor区間・有理平方根区間からCayley parameterを構成し、実現角誤差が入力角度の隣接binary64間隔の4分の1未満であることを証明する。`RᵀR = I`、`det R = 1`、軸方向と両端点の不動を発行前に完全有理算術で再検証し、全演算へ固定の項数・反復数・shift・bit長上限を適用する。

第二checkpoint内の非公開tree合成核として`rational_cayley_tree_pose_v1`を実装した。`BoundMaterialTreePose`から同一issuer、exact pose instance、fixed face、canonical face・hinge・角度・面境界を再認証し、既存solverと同じEdgeId順BFSおよび山谷×左右の整数符号で全局所回転を合成する。全face変換の`RᵀR = I`と`det R = 1`、全VertexId occurrence、共有ヒンジ両端および中点のBigRational完全一致、root変更時の単一global exact合同、source格納順不変性、ABA拒否を検証する。全face境界は一度だけ走査して認証済み`(FaceId, EdgeId)`索引を作り、ヒンジごとの全境界再走査を禁止する。局所演算に加えてtree全体のface・hinge・隣接・境界・索引・一意頂点・Machin/Taylor項・sqrt反復・演算・一時値を含む個別出力bit・分子と分母を合算した総出力storage bitへ、増分前の固定上限を適用する。rendererのbinary64姿勢をこのexact姿勢へ結ぶ有限誤差包含、deep-chain stressおよび厚さ0classifierへの接続は未実装なので、第二checkpoint全体、既存のcollision safe set、複数面production proofおよび完成率はまだ広げない。

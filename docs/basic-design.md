...
- 将来の`physical_grab_v2`は、折り面上の把持点とポインター投影から目標姿勢を求める別契約とし、`vertical_parameter_v1`の意味を置き換えない。
...
静的分類単独では連続運動を保証しない。単一ヒンジの角度操作は8.3の別ジョブで連続経路を判定し、現在姿勢の分類と連続経路の結果を別々に表示する。複数ヒンジの連続運動は未検証として扱う。いずれも実際の折り癖、材料変形、層ずれを保証せず、OQ-002の物理的な厚さオフセット規則は引き続き未解決とする。UIでも中央面基準モデルによる許容であることを明示する。

### 8.3 衝突前停止

最終目標では、現在角から目標角までの連続運動を判定し、連続安全を証明できた下限と、その直後の危険または未確定な探索区間を求める。確認済み境界で停止し、対象面、接触点、法線、折り角、分類をUIへ返す。

現在の初期実装は`centered_mid_surface_v1`による単一ヒンジだけを対象とし、現在の実表示角から指定角までを次の線形経路で扱う。
...
- `vertical_parameter_v1`の開始角には指定値ではなくrunnerの実表示角を使う。ドラッグ移動中は未確認目標だけを表示し、pointerup時に1回だけrunnerへ要求する。3D描画はrunnerが認定した角度だけを受け取る。
- ドラッグ開始前に画面投影したヒンジの可視深度、最小表示長、ポインター距離を再検証し、pointer captureを取得してからOrbitControlsを無効化する。複数pointer、横優勢gesture、capture喪失、blur、resize、範囲外、revision・固定面・紙厚の変更では要求を送らず取消す。
...
- `clear`表示にも中央面基準・単一ヒンジ・線形経路だけの確認であることを可視表示し、材料変形、実際の折り癖、層ずれ、複数ヒンジ、物理的な`physical_grab_v2`は未保証とする。

現在の停止位置は「最初の衝突時刻の推定値」ではなく、時刻順探索で最後に連続安全を確認できた下限である。接触点、法線、修正候補の追加、`physical_grab_v2`、複数ヒンジの連続運動は次段階とする。

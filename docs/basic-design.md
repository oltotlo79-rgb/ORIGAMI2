...
紙厚の半分を`h=t/2`、左右中央面法線間の角度を`θ`とすると、理想的な中央面基準half-slabの共有辺周辺重なりは半径`R=h/cos(θ/2)`に収まる。この解析条件を満たす共有辺三角形対だけを「モデルで許容した折り目境界接触」または「モデルで許容した折り目領域内重なり」とする。ただし、共有ヒンジの有限長を`L`とすると、初版がこの近似を許容するのは数値marginを含めて`R <= L + margin`を証明できる範囲までである。これを超える深角、または正厚の厳密な180度は、無限に広い許容corridorへ拡張せず`layer_offset_unmodeled`として判定不能で停止する。連続経路でも同じ有限半径条件を区間認定へ適用し、停止詳細には「層ずらし未再現」を専用理由として表示する。
...
|---|---|---|---|---|
| 厚さ0 mm | 接触 | 接触 | 接触 | 貫通 |
| 厚さ0.1 mm | 接触 | 接触 | 貫通 | 判定不能 |
| 厚さ3 mm | 接触 | 接触 | 貫通 | 判定不能 |

| 左だけを折り、右を0度に保つ | 10度 | 90度 | 179度 | 180度 |
|---|---|---|---|---|
| 厚さ0 mm | 接触 | 接触 | 接触 | 判定不能 |
| 厚さ0.1 mm | 貫通 | 貫通 | 貫通 | 判定不能 |
| 厚さ3 mm | 貫通 | 貫通 | 貫通 | 判定不能 |

片側だけを折る正厚ケースでは、中央面の両側へ紙厚を等分する初版近似により、折り線を直接共有しない外側2面のslabが共通V頂点の近傍で実体積交差するため貫通とする。正厚180度は層ずらし未再現、厚さ0・片側180度の外側2面は斜め軸変換後のsub-margin共面境界を安全に確定できないため判定不能とする。
...
共有ヒンジの有限軸証明では、三角形対の少なくとも片方が両端点間へ完全に収まれば、その交差集合も両端点間に収まるものとする。両方とも端点外へ伸びる組はこの証明を得ず、端点外のconcave lobeを許容しない。厚さ0かつ厳密180度以外は、検証済み共有軸、対向する材料半平面、有限軸の3証明がそろった数値的に限界の三角形対を境界接触として解決する。厳密180度は同じ証明範囲内だけを許容平坦積層とする。
...
- 狭域結果の`witnessSamples`は、最終interactionが`touching`または`penetrating`へ確定した非隣接triangle-pairだけをface ID・両triangle index・classと結合する。hingeと`indeterminate`は対象外とする。厚さ0の確定pairもcoverage上はeligible/attemptedとして数えるが、正厚三角柱witnessを作れないため`unavailable`となりsampleは返さない。witness導出が`null`でもauthoritative分類を変更しない。

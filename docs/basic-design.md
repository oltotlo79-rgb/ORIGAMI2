...
厚さ0のnative下位classifierは、有限binary64の三角形頂点を`2^-1074`単位の整数へexact変換し、離間・点接触・境界線接触・共面正面積・横断交差・判定保留を頂点順と面交換に依存せず分類する。認証済み共有点または完全共有辺上だけの接触は貫通へ昇格させず、共有要素外へ広がる共面正面積または横断は貫通証拠とする。退化、割当失敗、証明不足は判定保留へ閉じる。ただし、全faceの単純境界、凹多角形を含むexactな決定論的三角形分割、全triangle-pair coverageの証明が完成するまで、このclassifierから複数面のproduction geometry proofを発行しない。

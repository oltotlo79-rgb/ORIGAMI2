...
- EDT-009 frontend strict DTO・表示: 当該commit時点のNode 18/18、DOM 12/12、`tsc -b`成功。9種すべてをexact-key・canonical UUID・当時の最大3-ID witnessで検証し、追加2種も日英のblocking原因表示へ接続した。これは当該時点の履歴であり、後続の現行正本は最大256-IDのgeneral-graph原因を含む17 variant（13 pairwise + 4 general-graph）である。現行境界は`docs/requirements-status.md`を参照する。

...
辺の削除・分割・交差接続・T字接続・交差cluster・輪郭編集のinverseは、
生成または除去された辺の明示layer assignmentも保持する。fieldが存在しない旧V1
entryはimplicit default layerとして読める一方、保存されたassignmentの欠落、
追加、edge IDまたはlayer IDの改変は意味再認証で拒否する。
...
1. `project.json`からpattern、paper、geometric constraints、instruction timeline、
   project layersを含む現在文書を構築し、bit-exactな期待値を保持する。

...
- `vertices_coords`と`edges_vertices`を必須とする。`edges_assignment`は任意であり、省略時は全辺を`U`（unassigned）として扱って警告する。記載時は`edges_vertices`とのparallel array長、値を検証する。
- 頂点・辺の添字、有限値、退化辺、重複辺、交差候補の作業量を検証する。
- 頂点座標は`[x, y]`またはZが0の`[x, y, 0]`だけを許可する。非ゼロZ、`foldedForm`、`frame_attributes`の`3D`、複数frameは取り込まない。
- 全`B`辺が、各頂点の次数2、正の面積、自己交差・分断のない単一の単純閉路を作る場合は、その閉路を用紙境界として固定する。
- `B`がない、または全`B`辺だけでは有効な閉路にならない場合は、全辺が共有端点以外で交差しない平面グラフであることを検証し、その面巡回から「単純・正面積・全source頂点を内部または境界上に含む」外周候補を決定論的に列挙する。最大面積、凸包、線種、格納順から一つを推測せず、利用者が候補を明示選択する。候補がない場合または64件を超える場合は拒否する。
- 推定候補を選んだ場合、候補を構成する辺をassignmentにかかわらず用紙境界へ上書きし、候補外のsource `B`辺は取り込まない。穴、複数紙、複数境界は未対応である。
...
|---|---|---|
| `B` | boundary | 全`B`辺が有効な単一閉路なら用紙境界に固定。不成立時は外周候補を明示選択し、候補外の`B`辺は無視 |
| `M` | mountain | 山折りに固定 |
| `V` | valley | 谷折りに固定 |
...
選択した外周候補の辺は、上表のassignment割当より優先して用紙境界になる。`edges_assignment`省略時は、選択した外周候補以外の全辺に`U`の選択を適用する。

`F`、`U`、`J`はORIGAMI2に同義の線種がないため自動確定しない。取り込む辺に対する未選択が1つでもあれば適用できない。`C`を切断線へ割り当てた場合は、取込projectの切断許可を有効にする。

## 5. プレビューと警告

- native側で全入力を検証してから、作品名候補、仕様、単位、頂点・辺の件数、assignment別件数、検証済み外周候補、SVG線プレビューを返す。有効なsource `B`閉路がある場合は、その候補を固定して選択変更を要求しない。
- source `B`閉路を固定できない場合は、各候補の由来と辺数を表示し、利用者が一つを選択するまで適用できない。最大面積などによる既定選択は行わない。
- プレビューは最大5,000辺で、外周候補を構成する辺を先に決定論的に収容し、残りをassignment別に間引く。候補の一覧と完全な辺索引は表示間引きと独立して保持し、表示の間引きは取込本体を削減しない。
...
3. UIで作品名、縮尺、必要な外周候補、線種割当、警告確認を完了する。
4. 適用直前に現在projectが未保存なら、破棄確認を行う。
5. Rust側で同じbytesを再解析し、選択されたpreview-localな外周候補IDが再生成した候補集合に存在することを再検証してから変換する。候補IDは現在の取込IDが指すstage内でだけ解釈し、再解析後に存在しない候補IDは受理しない。併せてstage時のproject instance、project ID、revisionと現在値を照合する。

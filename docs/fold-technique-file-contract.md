...
初期editorでactionを変更するときは、変更前action固有の必須capabilityだけを除き、変更後action固有の必須capabilityを追加する。`instruction_timeline_v1`と`manual_pose_registration_v1`等のaction非固有capabilityは保持し、重複を除いたcanonical順へ戻す。
...
- 保存IPCのJSON文字列について1 MiBちょうど/1 byte超過、過剰なraw構造深さ、上限内bytesの巨大collectionを直接入力し、typed document graphへ入る前の境界を確認する
...
ファイルpathとraw bytesはWebViewへ返さない。読込は1 MiBを確保前に確認し、最終componentを追跡しない同一file handleを開いて通常ファイル・再解析point・実サイズを再検証してから、`ori-instructions`正本readerへ渡す。保存IPCはTypeScriptで厳格検証・canonical化したcompact JSON文字列だけを受け渡し、native側ではtyped document graphのdeserialize前に1 MiB上限を検査する。その後`ori-instructions`正本readerによる構造深さ・closed schema・件数・意味整合の検証、決定的serialize、独立read-backを完了してから、保存先と同じdirectoryの検証済み一時fileを原子的に公開する。既存destinationがlinkへ差し替わってもtargetは開かず、そのdirectory entryだけをreplace/no-replace規約で処理する。

読込・保存はprocess単位のsingle-flightとし、blocking workerが呼出元futureより長く残ってもworker終了までgateを保持する。Appはrequest IDを照合し、取消・失敗・古い応答では現在の技法documentとeditorを変更しない。処理中の終了をblockし、未保存workspaceを取込または新規作成で置換する前、ならびに未保存editor draftを閉じる前に利用者確認を必須とする。draft判定はinvalidな途中入力も含み、アプリ終了時の未保存判定へ統合する。作成時は初期documentでも保存可能とし、既存documentの編集では実差分がない限り確定buttonを無効にする。ダイアログ終了後のfocusは新規作成buttonへ固定せず、実際に開いたbuttonへ復元する。

1ファイル内の技法IDは最終確定時に一意でなければならないが、編集中は一時的な重複を許容してvalidation errorを表示する。選択UIのcomponent identityには編集可能なIDではなく安定したslot indexを使い、重複途中でも別技法へ誤って再対応付けしない。

この利用経路によりINS-009を実装済みとする。共有documentは引き続き純粋な宣言dataであり、project変更、折り動作、code実行、外部取得の権限を持たない。

## 10. タイムライン説明案への変換

INS-008の初版適用経路は、選択した一技法を自動実行せず、説明専用の`declarative_only_v1`手順案へ変換する。

変換順は次で固定する。

1. package metadataと技法名・説明
2. parameter定義
3. named precondition
4. 元文書のordered operation

各source objectは決定的なcompact JSONを`source-json-v1`として説明へ埋め込み、localized text、設定値、前提条件、binding、capability、execution supportを欠落させない。一説明が4,000 Unicode scalarを超える場合だけ連続chunkへ分割し、元sourceの順序を変えない。timeline全体512件またはIPC 2 MiBに収まらない案は部分追加せず拒否する。

利用者は追加順、件数、未対応操作数と次の安全条件をmodalで確認してから確定する。

- 全手順は説明専用で、現在の3D姿勢を変えない
- 中割り、かぶせ、沈め折り、層選択は注意付き説明テンプレートに留める
- `straight_line_stacked_fold`も折り重ね物理commandを実行しない
- 確定はtimeline末尾への一つの原子的commandで、一回のUndo/Redo対象になる

modalは安全側の取消へ初期focusを置き、focus trap、Escape、busy中の二重確定防止を持つ。project instance、project ID、revision、元技法documentまたは選択indexが変わった案はstaleとして確定できない。失敗時は固定文言だけを表示し、案を閉じず、projectと履歴を変更しない。

説明専用手順は`.ori2`とProject Folderで`instruction_timeline_v1`に加えて`declarative_instruction_steps_v1`を必須宣言する。旧版は未知の必須機能として安全に拒否する。復旧とPDF/SVG書き出しは説明・注意を保持する。説明専用手順の単独3D表示は「実行可能姿勢なし」と明示して拒否し、実姿勢との混在timelineの段階再生では説明専用手順を物理適用せず、実姿勢だけを元の相対順序と元timeline上の表示番号を保って再生する。

この範囲でINS-008の宣言的timeline生成・明示確認・原子的適用を実装済みとする。技法から3D運動を生成すること、物理操作の自動実行、折り重ねcommand、中割り・かぶせ・沈め・層選択の運動生成は含めない。

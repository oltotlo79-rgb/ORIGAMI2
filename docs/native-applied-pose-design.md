...
全pairの厚さ0診断集約はここまで実装したが、watertightなexact rigid pose、厚さ0の有限ヒンジ許容および正厚証拠が未完成なため、productionの複数面geometry proofは引き続きblockingとする。斜め共有featureのraw exact-affine端点不一致時にrawの横断または共面正面積をそのまま肯定貫通へ優先する暫定案は採用しない。任意に小さい端点残差でも、本来は共有ヒンジだけで接する二面に偽のrelative-interior横断または薄い共面重なりを作れるためである。watertight canonical pose、またはrawからcanonicalへの全face誤差包含と共有featureからの分離下限が証明されるまでは判定保留へ閉じる。

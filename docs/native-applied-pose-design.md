...
ヒンジ角はcanonical EdgeId順に保持し、`-0` は `0` へ正規化する。各値は有限かつ `0..=180` とする。
...
- geometryまたはtopologyを変える編集
- Undo/Redo
- projectのnew、open、replace
- current poseの明示的invalid化
- authority slotの差し替え

overflow時は新しいauthorityを発行せず、既存状態を完全に維持してfail closedとする。generationをJS numberとして公開する場合はsafe integer上限を超えない規約を使用する。
...
semantic poseは最終的に `ori-core::EditorState` の履歴対象とする。in-process certificateは `ProjectState` 配下のauthority slotで管理し、persisted documentへopaque fieldやprocess identityを保存しない。
...
固定したlock順で、次のimmutable snapshotとcapabilityを取得する。
...
commit直前に同じ固定lock順で必要なlockを取得し、次を再照合する。
...
ただし、古いworkerやcapabilityを再びcurrentにしてはならない。そのため、Undo/Redoでもrevision、pose generation、layer-order generation、必要なauthority generationをchecked単調増加させ、新しいcertificate objectを発行する。
...
- edit、Undo、Redo時のsemantic poseとauthority
...
`topology_contact_policy_v1`の純粋な4×10 decision表は、frontendとnativeが同じ正規JSON corpusへ全40セルを照合する形で先行実装した。この段階は入力evidenceを証明せず、共有ヒンジを許容せず、衝突certificateも発行しない。次の先行テストと実装で、current native poseに束縛した共有関係・交差証拠の肯定証明、全pair coverage、有限ヒンジモデルおよび作業上限を追加して初めてCのcertificate境界とする。
...
- frontendの4×10分類表と同じcanonical fixture corpus

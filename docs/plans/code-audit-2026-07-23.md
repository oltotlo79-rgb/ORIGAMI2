...
- 後続監査で、`horizontal_and_vertical`の従来2-ID原因は同一辺を長さ0へ潰す解を持ち、soundな直接矛盾ではないことが判明した。`7c7134d`から修正`6b00ed0`までの「13種のsoundな矛盾」という文書表現は、この1種について過大だった。
- `6b00ed0`では実solver residualが水平`dy=0`・垂直`dx=0`を同時に満たすzero-length escapeを回帰し、同じ辺の正の固定長を含む3-ID原因だけを直接矛盾として残した。固定長がなければ判定保留へ閉じ、旧2-ID DTOを拒否、新3-ID DTOと削除最小性を検証した。WSL focused 2件、削除最小1件、Clippy all-targets、Node 18件、DOM 12件、production buildが成功したため、修正後の13種はsoundと再評価できる。一般MUSは未実装なのでEDT-009部分実装と全体79.32%（表示79.3%）は変更しない。

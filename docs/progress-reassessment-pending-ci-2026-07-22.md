...
| 初心者向け自動設計 | 8% | 55% | 60% | 4.80% | bounded custom target・一般木候補・画像/GLB consensus。一般目標からの設計生成とは数えない |
| **合計** | **100%** | — | — | **83.96%** | **表示84.0%** |
...
+ 14.04 + 9.20 + 5.00 + 3.75 + 4.80 = 83.96%
```
...
このため83.96%は「残件が小さい」ことを意味せず、一般物理motion、一般自動設計、正式配布という高難度の終盤作業を明示的に残した保守的な工数概算である。

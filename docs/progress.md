...
| 2D展開図エディター | 15% | 36% | 5.40% | 基本編集、計測、任意多角形用紙の境界編集、補助線、交点接続と8種のスナップを実装 |
| 数式・幾何制約 | 9% | 38% | 3.42% | 完全な紙境界検証、矩形生成、比例変換、辺・X交点・T字交点分割、方向・平行スナップの異常値防御を実装 |
...
| 多言語・設定・配布・QA | 5% | 24% | 1.20% | 両OS CI、正式形式のアプリアイコン、macOS bundle検証、10万件ファズを含む幾何・スナップ・性能テストを追加 |
...
- スナップ単体・性能テスト59件と、X交点・T字交点編集を含むori-coreテスト67件
...
3. 角度スナップを追加し、水平・垂直・平行との決定的な優先順位を完成させる
4. 輪郭辺との交点と、3本以上が同一点へ集まる交点の一括接続を設計する

...
- 衝突分類のproduction接続前checkpointとして、`MeasuredBinary64AffineEnvelope`を測定元のexact `E`へ安全なborrow identityで束縛し、同じboundから独立再生成した`E1/E2`の組替えを拒否した。actual mmの閉有理区間だけを受けるprivate blocking-only横断primitiveは、strictな内部横断だけを`ProvenPenetrating`とし、接触・共面・共有ヒンジ・不正入力・資源超過を`Unresolved`へ閉じる。共有頂点ではincident edgeを除外し、共有点外の反対edge横断だけを肯定できる。Cayley核はcanonical equalityをallocation-free構造比較、大小順序をmetered比較へ分離し、clone・符号反転、interval順序、dyadic変換、平方根、binary exponent shiftを有限workへ含めた。実カーネルと実treeでもGCD・allocationの5上限を同時に実測exactへ合わせた成功と各one-shortのfail-closedを固定した。`ori-collision`はlib 122件をDebug/Release、blocking-only 13件をDebug/Release、静的衝突integration 5件をRelease、v2公開表integration 2件をDebug、doc 1件をReleaseで通過し、全target・全featureのRelease check、Clippy `-D warnings`、fmtも通過した。v2公開表integrationのRelease実行だけはWindows Application Control（OS error 4551）に阻止されたためCIで再検証する。このprimitiveは`E` / envelope / topology / pair bridge、実400 mm姿勢、正厚、renderer、certificate、public safe proofまたはSIM-010へ未接続なので、MUST集計32 / 27 / 28と完成率36.9%は変更しない

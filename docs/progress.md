...
**全体完成率: 約23.7%（2026-07-17）**

完成率は画面数ではなく、要件定義書のMUST 86件、FUTURE 14件、品質検証、両OS配布を含む総工数の概算である。研究要素の結果によって見積もりを更新する。

下表の「全体への寄与」は「全体比率 × 現在の領域進捗」であり、合計23.67%を小数第1位へ丸めて全体完成率としている。

## 重み付け

| 領域 | 全体比率 | 現在の領域進捗 | 全体への寄与 | 状態 |
|---|---:|---:|---:|---|
| 要件・基本設計・技術検証 | 5% | 63% | 3.15% | 面抽出・平面トポロジーとmacOS終了・復旧保護まで設計を文書化 |
| プロジェクト・保存・履歴 | 8% | 60% | 4.80% | 原子的編集とUndo/Redo、ネイティブ`.ori2`実ファイル操作、全OSの原子的上書きと失敗時保護を実装 |
| 2D展開図エディター | 15% | 48% | 7.20% | 基本編集、任意多角形用紙、9種のスナップに加え、単一折り線の二面・ヒンジ抽出を実装 |
| 数式・幾何制約 | 9% | 46% | 4.14% | 厳密な面積・向き判定を単一折り線の弱凸判定と決定的な左右面構築へ適用 |
| 3D折り・紙厚・衝突 | 17% | 9% | 1.53% | revision検証済みの実展開図から面プリズムを作り、実ヒンジ軸と山谷方向に従う単一折り線3D表示を実装 |
| 折り可能性・経路探索 | 18% | 0% | 0.00% | 未着手 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 39% | 1.95% | 両OS CI、実ファイル回帰、候補ファズに3D入力のfail-closed検証・三角形・法線回帰を追加 |
...
- frontend単体・性能テスト137件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・revision付き面解析を含むori-coreテスト92件
...
- 補助線の交差・重複・未解決端点を面構築から除外しつつ、重複ID、複数折り線、凹紙、内部終端、切断線を構造化診断で安全側に遮断
...
1. 複数折り線を含む決定的half-edge rotation systemを実装する
2. half-edge walk列挙、紙外部の特定、凸紙内の複数面分類を実装する
3. ネイティブ`.ori2`のダイアログ以外を自動E2E化し、Windows/macOS実機でダイアログ操作を回帰試験する
4. macOSのDock／OS終了時の未保存保護とクラッシュ時自動復旧を実装する
5. Windows/macOSの基準PCで1万辺の実測値を記録し、性能合格値を確定する

...
| 数式・幾何制約 | 9% | 56% | 5.04% | 非可換な木構造でも選択ヒンジ以外を固定した従属面群が1本の円軌道になる準備契約を実装 |
| 3D折り・紙厚・衝突 | 17% | 42% | 7.14% | 単一折りUIに加え、木構造の選択ヒンジ・従属面群の物理把持準備と重なり面pickingを実装 |
...
| 多言語・設定・配布・QA | 5% | 61% | 3.05% | 両OS CI、実ファイル回帰に加え、物理把持・副作用順序・入力境界を含むfrontend 429件、Rust 302件を検証 |
...
- frontend単体・性能テスト429件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むRustワークスペーステスト302件
...
1. 木構造の選択ヒンジ1本だけを動かす連続衝突解析を実装し、他ヒンジを固定した認定済み角度だけをUIへ適用する

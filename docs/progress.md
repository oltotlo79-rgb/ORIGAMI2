...
**全体完成率: 約44.2%（2026-07-20、暫定の重み付き概算）**
...
下表の「全体への寄与」は「全体比率 × 現在の領域進捗」である。直前値43.11%に対し、LIN-004の作成・改名・並べ替え・削除・選択折り線の割当をproject instance・project ID・revisionへ束縛した管理UIとして接続し、「2D展開図エディター」を51%から54%へ更新した。さらに、固定GitHub Releases APIへの明示的な手動更新確認、日英状態表示、端末ごとの有効・無効設定、通信内容と自動ダウンロード・自動インストールを行わないことの説明を利用者経路へ接続し、OPS-001/002/003を実装済み、「多言語・設定・配布・QA」を60%から72%へ更新した。差分は0.45ポイント（全体比率15% × 3ポイント）と0.60ポイント（全体比率5% × 12ポイント）の合計1.05ポイントで、44.16%を小数第1位へ丸めて表示している。注釈・下絵objectの編集、layerの表示・lock・透明度、GitHub Releasesへの正式版公開workflowは未実装である。正厚の共面重なり・境界面接触・正体積を含む完全な三角柱分類、共有ヒンジ一般、native診断による操作の巻戻し・連続経路停止、専用の層順3D viewer、SIM-010の折り重ね、一般経路探索、FOLDの3D・複数frame、3D完成形出力は計上していない。水平・垂直以外の幾何制約作成UI、拘束を満たす自動変形、非線形・推移的矛盾の完全solverも未実装である。privateな正厚三角柱交差核、exact E/F有限ヒンジ回廊証明、完全平坦180度に限定した多面層順序anchorは利用者経路へ未接続なので、それ自体は加算していない。入力値自体が概数なので、44.16%は追跡用の計算値であって測定誤差のない精密値ではない。

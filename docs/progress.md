...
**全体完成率: 約26.6%（2026-07-17）**

完成率は画面数ではなく、要件定義書のMUST 86件、FUTURE 14件、品質検証、両OS配布を含む総工数の概算である。研究要素の結果によって見積もりを更新する。

下表の「全体への寄与」は「全体比率 × 現在の領域進捗」であり、合計26.56%を小数第1位へ丸めて全体完成率としている。

## 重み付け

| 領域 | 全体比率 | 現在の領域進捗 | 全体への寄与 | 状態 |
|---|---:|---:|---:|---|
| 要件・基本設計・技術検証 | 5% | 66% | 3.30% | 複数折りの面抽出に加え、hinge graphの木・閉路分類と安全なconsumer境界まで実装検証 |
...
| 3D折り・紙厚・衝突 | 17% | 16% | 2.72% | 木構造の複数ヒンジで親子の累積剛体変換を伝播し、全材料面とヒンジを角度操作へ接続 |
...
| 多言語・設定・配布・QA | 5% | 43% | 2.15% | 両OS CI、実ファイル回帰に加え、木構造の累積変換・閉路退避・非可換回転を含むfrontend 147件を検証 |
...
- frontend単体・性能テスト147件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むori-coreテスト95件
...
1. ヒンジごとの角度状態と編集UIを追加し、木構造の各関節を独立操作できるようにする
2. 自己衝突・紙同士の貫通を検出する広域判定を追加し、折り可能性と経路探索へ接続する
3. 閉路拘束の診断を詳細化し、将来の閉路ソルバーへ渡す入力・出力境界を定義する
4. 大規模な面数に向けて変換行列を再利用し、角度更新を描画フレーム単位に集約する
5. ネイティブ`.ori2`の自動E2EとWindows/macOS実機回帰、macOS終了時の未保存保護を進める

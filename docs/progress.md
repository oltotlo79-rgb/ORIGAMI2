...
| 3D折り・紙厚・衝突 | 17% | 44% | 7.48% | 木構造の従属面物理ドラッグを、完全姿勢の原子的シーン適用と認定済み角度の確定へ接続 |
| 折り可能性・経路探索 | 18% | 10% | 1.80% | 木構造の選択1ヒンジについて、他ヒンジ固定の連続衝突ジョブを3D物理把持UIから実行 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 65% | 3.25% | 両OS CI、実ファイル回帰に加え、木構造の物理把持UI統合を含むfrontend 499件、Rust 302件を検証 |
...
- frontend単体・性能テスト499件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むRustワークスペーステスト302件
...
2. 衝突位置・法線・修正候補を安全なwitness境界からUIへ提示する

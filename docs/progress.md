...
| プロジェクト・保存・履歴 | 8% | 41% | 3.28% | revision、順序保持Undo/Redo、完全な紙設定、ネイティブ`.ori2`読込・保存を実装 |
| 2D展開図エディター | 15% | 12% | 1.80% | 頂点・山谷線・切断線の追加、選択、ドラッグ、座標変更、削除を実装 |
| 数式・幾何制約 | 9% | 16% | 1.44% | 展開図と紙境界・紙厚の検証、矩形一枚紙生成、境界保護を実装 |
...
| 多言語・設定・配布・QA | 5% | 10% | 0.50% | 両OS CI、正式形式のアプリアイコン、macOS bundle検証を追加 |
...
3. 新規プロジェクト画面から矩形寸法、紙厚、表裏色、切断可否を指定できるようにする

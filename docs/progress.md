...
**全体完成率: 約43.1%（2026-07-20、暫定の重み付き概算）**
...
下表の「全体への寄与」は「全体比率 × 現在の領域進捗」である。直前値42.41%に対し、UI-001の日本語・英語切替を主要画面、ダイアログ、2D/3D、折り手順、ARIA、通知、既知native警告まで利用者経路へ接続し、「多言語・設定・配布・QA」を46%から60%へ更新した。差分0.70ポイント（全体比率5% × 14ポイント）を加えた43.11%を小数第1位へ丸めて表示している。LIN-004のversion固定layer文書、edge assignment、履歴、保存・復旧、strict IPC snapshotは内部からdesktop境界まで接続したが、layer管理UIと注釈・下絵object編集が未接続なので、追跡方針どおり完成率にはまだ加算していない。GitHub Releases更新確認は安全な通信・設定基盤までで利用者UIへ未接続のため、OPS-001/002/003と完成率にはまだ加算していない。正厚の共面重なり・境界面接触・正体積を含む完全な三角柱分類、共有ヒンジ一般、native診断による操作の巻戻し・連続経路停止、専用の層順3D viewer、SIM-010の折り重ね、一般経路探索、FOLDの3D・複数frame、3D完成形出力は計上していない。水平・垂直以外の幾何制約作成UI、拘束を満たす自動変形、非線形・推移的矛盾の完全solverも未実装である。privateな正厚三角柱交差核、exact E/F有限ヒンジ回廊証明、完全平坦180度に限定した多面層順序anchorは利用者経路へ未接続なので、それ自体は加算していない。入力値自体が概数なので、43.11%は追跡用の計算値であって測定誤差のない精密値ではない。
...
| 2D展開図エディター | 15% | 51% | 7.65% | 基本編集、9種スナップ、画面・取込・SVG書出で統一した白黒識別可能な5線種を実装。layer文書・edge assignment・履歴・保存・strict IPC基盤まで実装し、管理UI、注釈・下絵object、面編集、数式作図、対称編集を残す |
...
| 多言語・設定・配布・QA | 5% | 60% | 3.00% | frontend/Rustの自動回帰、Windows/macOS CI、redacted diagnostics、ライト/ダーク/OS連動、標準・変更可能shortcut、保存可能な作業レイアウトに加え、主要画面・ダイアログ・ARIA・通知を含む日英ライブ切替と端末保存を実装。更新確認UIとGitHub Releases正式配布を残す |

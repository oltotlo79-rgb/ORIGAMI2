...
| 3D折り・紙厚・衝突 | 17% | 32% | 5.44% | 広域・三角柱SAT診断、描画フレーム統合に加え、問題面を重要度別の3D輪郭で追跡可能 |
...
| 多言語・設定・配布・QA | 5% | 51% | 2.55% | 両OS CI、実ファイル回帰に加え、衝突表示の優先度・件数を含むfrontend 189件を検証 |
...
- frontend単体・性能テスト189件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むori-coreテスト95件
...
- ヒンジ隣接の接触・貫通を除外も許可認定もせず、厚さoffsetとhinge corridorを検証する後段ポリシー向けの未解決相互作用として保持
- 3D診断を非隣接貫通・非隣接接触・ヒンジ未解決・数値不確定へ色分けし、「現在姿勢のみ・連続運動は未検証」を明示
- 非隣接貫通面を赤、非隣接接触面を紫、数値不確定面を黄の3D輪郭で表示し、貫通・不確定・接触の順に固定面・従属面の強調より優先
...
1. 共有ヒンジの接触領域をhinge corridor・厚さoffset規則と照合し、許容接触とヒンジ外貫通を分離する
2. 現在角から目標角までの連続衝突判定と安全停止時刻を実装し、折り可能性へ接続する
3. 三角形分割・三角柱の再利用とworker分離により、大規模な面・ヒンジの狭域判定を最適化する
4. 閉路拘束の診断と将来ソルバー境界を詳細化する
5. 3Dカメラ操作、キーボード選択、ネイティブE2E、終了時保護を進める

...
| プロジェクト・保存・履歴 | 8% | 52% | 4.16% | revision、順序保持Undo/Redo、紙設定の単一状態管理、ネイティブ`.ori2`読込・保存、新規作成を実装 |
| 2D展開図エディター | 15% | 18% | 2.70% | 基本編集、可変寸法用紙、線の長さ・角度計測、用紙リサイズを実装 |
| 数式・幾何制約 | 9% | 23% | 2.07% | 完全な紙境界検証、矩形生成、全頂点の安全な比例変換、境界保護を実装 |
...
| 多言語・設定・配布・QA | 5% | 14% | 0.70% | 両OS CI、正式形式のアプリアイコン、macOS bundle検証を追加 |
...
3. 輪郭辺を分割し、新しい輪郭頂点を移動して任意多角形の紙を作成できるようにする

...
| 3D折り・紙厚・衝突 | 17% | 27% | 4.59% | 実紙厚の面プリズムAABBを姿勢ごとに更新し、ヒンジ隣接を保持した決定的な広域衝突候補を3Dへ表示 |
...
| 多言語・設定・配布・QA | 5% | 48% | 2.40% | 両OS CI、実ファイル回帰に加え、1万面広域判定・上限遮断・姿勢完全性を含むfrontend 171件を検証 |
...
- frontend単体・性能テスト171件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むori-coreテスト95件
...
1. 広域候補へ三角形・厚み形状の狭域判定を追加し、許容ヒンジ接触・接触・不正貫通を分類する
2. 変換行列の再利用と描画フレーム単位の角度更新により、大規模な面・ヒンジ操作を最適化する
3. 閉路拘束の診断と将来ソルバー境界を詳細化する
4. 3Dカメラ操作、面・ヒンジのキーボード選択、選択表示の視認性を改善する

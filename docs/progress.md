...
- `ori-collision`には単一material face・no-hingeだけを対象とする`NativeStaticCollisionGeometryProof`を追加した。全unordered face pairが厳密に0件であること、exact model issuer、pose instance、紙厚bits、`centered_mid_surface_v1`、policy/model/proof IDをopaque proofへ結合する。複数面は`PairEvidenceUnavailable`でblockingとし、runtime `same_face`は`indeterminate`へ閉じる。この値はproject/revision/current pose certificate/generationを持たない下位幾何証拠であり、設計Cのcurrent static collision certificateではない。折り重ね利用者経路は増えていないため完成率は36.9%のままとする
...
- `topology_contact_policy_v1`の共有関係4種×交差証拠10種・全40セルを互換基準として凍結し、正厚の正面積境界接触を独立させた`topology_contact_policy_v2`の4×11・全44セルも言語間の正規JSON corpusへ固定した。frontend/nativeの純粋表は一致し、v2は共有なし/共有頂点の境界面接触を`touching`、共有ヒンジを有限model必須、同一面をself除外とする。厚さ0から境界面証拠を発行しないこと、共有ヒンジを表だけで許容しないこと、runtimeのsame-face到達を内部不整合へ閉じることをnative境界へ固定した。単一面zero-pairの下位幾何proof以外の複数面証拠生成、有限ヒンジ許容、current静的・連続衝突certificateは未実装なので、SIM-010または完成率へ加算しない
...
1. 完成したdesktop current pose capabilityへ、単一面zero-pairのnative静的幾何proofをexact certificate identityとgenerationで再結合するcurrent static collision certificateを追加する
2. nativeの衝突分類v2 4×11純粋表へ、複数面の認証済み共有関係と幾何学的交差証拠、全pair coverage、有限ヒンジcorridorを追加し、続いてcurrent poseまでのcontinuous collisionと場所別cell-order transportを証明する。全180度flatは内部bootstrapに限定し、製品要件をflat限定へ縮小しない

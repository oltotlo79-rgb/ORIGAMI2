...
| 3D折り・紙厚・衝突 | 17% | 4% | 0.68% | 可変矩形の試作表示に加え、実展開図から二面・一ヒンジを生成する入力基盤を実装 |
| 折り可能性・経路探索 | 18% | 0% | 0.00% | 未着手 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 37% | 1.85% | 両OS CI、実ファイル回帰、候補ファズに単一折り線の黄金値・決定性回帰を追加 |
...
1. revision付き面・ヒンジ解析をTauri経由で3D表示へ渡し、実展開図の二面をヒンジ軸回りに表示する
2. 複数折り線を含むhalf-edge rotation systemとwalk列挙を実装する

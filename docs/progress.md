...
- EDT-009はwire互換の`DirectConstraintConflictKindV1` enum 21 variantを維持する一方、実際のbinary64 residualに対して`DirectConflict`へ昇格できるsound allowlistは6 variantだけである。残る15 variantは`Unknown`へfail-closedであり、全制約種の一般充足可能性、一般矛盾原因、一般MUSは未完成。
...
- 2026-07-26 EDT-009実残差追補: `LengthRatioWithIncompatibleFixedLengths`を、consistentな正の有限固定長2件と正の有限比率に限定し、solverと共有する`ratio * denominator`、`numerator - scaled`のbinary64演算順で残差が非ゼロまたは非有限の場合だけ3-ID直接矛盾へ昇格した。exact-real積が不一致でも丸め後残差がゼロになる最小subnormal境界は`Unknown`を維持し、0.3/0.1/3.0、underflow、overflow、削除最小、入力逆順、4/8/16/17制約、solver/verifierのpreflight優先を固定した。sound allowlistは6種、fail-closedは15種となるが、一般充足可能性判定と全制約種の一般MUSは未完成である。数式・幾何制約85%、全体81.96%（表示82.0%）、MUST集計85 / 2 / 0は変更しない
- SIM-010のbounded 2..=8 block正厚層authorityについて、同一live material geometryから発行したgap reportを消費し、canonical face/hinge完全union、hinge一意性、block間face共有tree、親authorityのblock列、source・厚さ・issuer context・層fingerprint・target角を再検証する非直列化sealed authorityを追加した。3/4/8 blockの実proofと、欠落・余剰・重複、cycle・disconnect、別instance、順序・厚さ・issuer・fingerprint・target角・binding改ざんの拒否を固定した。このauthorityはsubmitted setの完全被覆だけを証明し、cross-block continuous clearance、共通articulation pose、cross-block layer transport、Apply・project mutation・viewerを認可しないため、SIM-010は部分実装、全体81.96%（表示82.0%）、MUST集計85 / 2 / 0を維持する
...
- 初心者向け自動設計で、動物・昆虫カテゴリ、部品、棒状骨格、画像下絵、marker/silhouette認識案、突起属性、3D膨らみ目標、評価preset、生成制約、追加候補、弾性非計算方針、端末内の限定展開図候補をproject履歴とUIへ接続した。「初心者向け自動設計」を55%へ更新し、全体を79.32%（表示79.3%）とした
- version固定の注釈objectを実装した。absoluteまたは頂点追従anchor、text、font size、bold/italic、文字色・不透明度、annotation layerをstrict validationし、ProjectDocument・通常保存・復旧・主Editor履歴・Undo/Redo・履歴上限・layer lock/delete guard・anchor頂点削除guard・native strict IPCへ接続した。日英UIで作成・選択・編集・削除でき、Canvasはlayer visible/opacityと頂点anchorを反映して描画する。frontend production buildとDOM 242件が成功し、「2D展開図エディター」を65%、全体を62.87%（表示62.9%）へ更新した
...
- SIM-010の限定正厚treeを13 triangular faces・12 hingesへ拡張した。全78 face pairを有界memoで被覆し、4度以下でcontinuous certificateとlayer orderを発行する。角度超過、14-face cap超過、ABA pose、紙厚bit driftはfail-closedにし、成功・超過・再認証の統合試験は58.71秒、cap試験は0.01秒、desktop all-target checkが成功した。「3D折り・紙厚・衝突」を98%、全体を57.45%（表示57.5%）へ更新した
- SIM-010の限定正厚treeを12 triangular faces・11 hingesへ拡張した。全66 face pairを有界memoで被覆し、5度でcontinuous certificateとsafe-stopを発行し、上限超過、13-face cap超過、ABA pose、紙厚bit driftはfail-closedにする。成功fixtureは43.45秒、対象3試験は36.92秒、desktop all-target checkが成功した。「3D折り・紙厚・衝突」を96%、全体を57.11%（表示57.1%）へ更新した
- 11種の幾何制約へ有界Gauss-Newton solverを接続した。頂点移動と辺の平行移動・回転・長さ変更をmulti-driverで解き、previewにrank・自由度・方程式数・条件推定・過不足分類を表示する。opaque tokenだけをApplyへ渡し、project instance・ID・revision・layer lockを再検証して1件の履歴へ原子的に確定する。stale拒否、lock時無変更、履歴1件、Undo/Redo bit完全復元のnative 4件、core solver、frontend DOM、production build、workspace checkが成功した。「数式・幾何制約」を55%、全体を56.77%（表示56.8%）へ更新した
- SIM-010の限定正厚treeを11 triangular faces・10 hingesへ拡張した。全55 pairをmemo cap 55で完全被覆し、6度でcertificate・safe-stop requested、6度超と12-face cap超過をfail-closedにする。positive layer orderを(11,10)へ限定admitし、runtime群32.37秒、desktop checkが成功した。「3D折り・紙厚・衝突」を94%へ更新した
- 全11種の幾何制約作成UIを接続した。fixed length/angle、horizontal/vertical、equal length、parallel、point on line、mirror/rotational symmetry、angle bisector、length ratioをtyped formから作成できる。canonical edge/vertexをrole別に選択し、数値をstrict validation後、project instance/revision固定のnative atomic commandへ送り、history・UndoRedo・保存・復旧・stale guardを共用する。frontend 1698件、build、lintが成功した。「数式・幾何制約」を30%、全体を54.52%（表示54.5%）へ更新した
- SIM-010の限定正厚treeを10 triangular faces・9 hingesへ拡張した。全45 pairをmemo cap 45で完全被覆し、8度でcertificate・safe-stop requested、8度超と11-face cap超過をfail-closedにする。positive layer orderを(10,9)へ限定admitし、runtime群33.67秒、desktop checkが成功した。「3D折り・紙厚・衝突」を92%、全体を53.46%（表示53.5%）へ更新した
- SIM-010の限定正厚treeを9 triangular faces・8 hingesへ拡張した。全36 pairをmemo hard cap 36で完全被覆し、10度でcertificate・safe-stop requested、10度超と10-face cap超過をfail-closedにする。runtime 3件、memo cap、desktop checkが成功した。「3D折り・紙厚・衝突」を90%へ更新した
- 正式版配布workflowを追加した。trusted GPG signed tag、version/HEAD一致、Windows Authenticode/NSIS、macOS hardened-runtime codesign、CycloneDX SBOM、全payload checksum、GitHub provenance attestation、prereleaseからstableへの無再build promotion、権限分離、既存release上書き拒否を実装する。default dry-runとlocal artifact contract 3件が成功し、実公開は行っていない。「多言語・設定・配布・QA」を75%、全体を53.12%（表示53.1%）へ更新した
- IO-008の正厚STLをPrusaSlicer 2.9.6固定版で実機受入した。公式配布物checksumを検証し、10×10×2 mm solidをCLIでmanifold、1 part、12 facets、約200 mm³、repair 0、warning/error 0として読込む。open sheetはload失敗、duplicate-face meshはrepair検出で期待rejectし、G-code 6 layersとfinite XYZ boundsも確認する。これによりIO-008を実装済み、「入出力・互換性」を100%へ更新した
- SIM-010の限定正厚treeを8 triangular faces・7 hingesへ拡張した。各hingeの認証済みprojectionをtree boundaryへ直結して重複exact計算を除去し、全28 pairをbounded memoで被覆する。15度でcertificate・safe-stop requested、15度超でfail-closed、8-face runtime 22.19秒、desktop checkが成功した。「3D折り・紙厚・衝突」を88%、全体を52.63%（表示52.6%）へ更新した
- IO-008の生成物をBlender 4.5.11 LTS固定版でCI実機受入した。checksum検証済み配布物を`--background --factory-startup --disable-autoexec`で起動し、static OBJ/STL/GLB、textured GLB、animated GLBをimportする。頂点・triangle、material・image、action/morph playback、open-sheet manifold metadata、OBJ/STLのmm bounds、GLBのmeter/axis boundsを検証し、Windows実機でも5件を確認した。slicerは別残件として維持する。「入出力・互換性」を99%、全体を52.24%（表示52.2%）へ更新した
...
- SIM-010の限定正厚treeを4 triangular faces・3 hingesへ拡張した。全hingeのcanonical projection/outer-shell observation、shared-vertex junction、全nonadjacent prism pair separationを同一pose・厚さへ完全被覆し、positive layer orderを(4 faces, 3 hinges)へ限定admitする。strict hexagon-fan fixtureの10/30/60度でcertificate・safe-stop requestedを発行し、60度超はfail-closedにした。既存positive transaction/atomic Apply/UndoRedo経路を共用する。runtime certificate testとdesktop checkが成功した。「3D折り・紙厚・衝突」を80%、全体を51.12%（表示51.1%）へ更新した
- SIM-010の厚さ0adaptive interval theoremを33〜64 hingesへ拡張した。path-wide conservative expansionでcanonical candidate setを一度だけsweep生成し、各leafは固定candidate arrayだけを走査する。candidate 2048、work 262144相当の有限上限で、48-hinge sparse success、64-hinge dense拒否、小規模brute-force一致、入力順決定性を回帰した。さらに限定正厚を3 triangular faces・2 hingesへ拡張し、canonical shared world axis、2 local outer-shell observations、shared-vertex junction、nonadjacent prism separationを完全被覆して10/30/45/60度でcertificate・safe-stop・positive layer order・transaction readyを発行する。desktop check、continuous tests、frontend回帰が成功した。「3D折り・紙厚・衝突」を78%、全体を50.78%（表示50.8%）へ更新した
- SIM-010のadaptive interval theoremを17〜32 moving hingesのsparse treeへ拡張した。interval-expanded face AABBをx-min・FaceIdで決定sortするsweep-and-pruneに置き換え、active x-overlap候補だけy/zを評価し、breakで省略したpairもx strict gapをcoverage marginへ含める。per-interval candidate 512、総work 65536のhard limitでdense caseはfail-closedにする。24-hinge sparse success、32-hinge dense cap拒否、入力逆順決定性、既存小規模全pair一致を回帰した。frontend 1458件・DOM 7件、desktop checkが成功した。「3D折り・紙厚・衝突」を74%、全体を50.10%（表示50.1%）へ更新した
- SIM-010の非同軸tree interval theoremをadaptive subdivision付き3〜16 moving hingesへ拡張した。interval評価は全非隣接pairのaxis separationから最小strict marginを保持し、未証明queueを最小margin・lower endpoint・depth順で二分する。leaf 128、depth 7、pair 136、pair-work 17408のhard limit、checked overflow、非有限、budget exhaustionをfail-closedにする。診断/UIへleaf/work metadataを追加し、9-hinge deep success・入力逆順一致、16-hinge budget/depth拒否、near collisionを回帰した。native tests、frontend 1458件・DOM 7件が成功した。「3D折り・紙厚・衝突」を72%、全体を49.76%（表示49.8%）へ更新した
- SIM-010の厚さ0非同軸continuous interval theoremを3〜8 moving hingesのtreeへ拡張した。rootからface ancestry depthを求め、各interval midpointの完全poseでface別AABBを`depth × global hinge radius × half interval radians`だけ膨張し、全非隣接pairがstrict分離する場合だけcertificateを発行する。hinge 8、pair 36、interval×pair 2304のhard limit、checked overflow、非tree、不完全pose、near collisionをfail-closedにし、新model IDをnative/UIへ接続した。3-hinge success、near-collision拒否、all-target check、frontend 1458件・DOM 238件が成功した。「3D折り・紙厚・衝突」を70%、全体を49.42%（表示49.4%）へ更新した
- SIM-010を厚さ0の限定非同軸2-hingeへ拡張した。3 face・2 moving hinge・initial全0を対象に、各angle interval midpointの全face AABBを2 hinge合成の解析的Lipschitz変位上限で膨張し、全intervalで全非隣接face pairがstrict分離する場合だけcontinuous certificateを発行する。samplingはblocking witness観測だけに使い証明根拠へはしない。indeterminate・overlapはfail-closedとし、UIへcertificate model、first proven blocking sample、disabled理由を接続した。continuous 4件、native 7件、frontend 9件・DOM 7件、build、all-target checkが成功した。「3D折り・紙厚・衝突」を68%、全体を49.08%（表示49.1%）へ更新した
- current global flat-foldability snapshotを専用3D層順viewerへ接続した。native read-only commandはproject instance・ID・revisionとcurrent layer capabilityを再認証し、generation・cell hash・bottom-to-top FaceId・finite world境界だけを返す。possible結果から候補操作なしで既存viewerを表示し、2D/3D face選択、cell focus・hoverを同期する。authority・revision・result変化時は表示状態を破棄する。native、frontend 5件、Global/Stacked DOM 23件、build、lint warning 0が成功した。「3D折り・紙厚・衝突」を66%、全体を48.74%（表示48.7%）へ更新した
- SIM-010を限定正厚single hingeと厚さ0同軸multi-hingeへ拡張した。正厚は2三角形face・1hinge・90度以下でouter shell、static classifier、continuous safe-stop、positive layer orderを同一bound pose・厚さへ束縛し、UIにproof model・厚さ・限定条件を表示して既存atomic Apply/Undo/Redoへ接続する。同軸multi-hingeは全moving hinge初期0、world軸同一直線、half/endpointで非fixed全faceがbit-exact同一transformの単一rigid body、全sample nonblockingの場合だけcertificateを発行する。cycle nonclosing・closure済みpath未証明・別軸は日英固定理由でdisabledにする。frontend 1457件・DOM 238件、continuous 3件、native 7件、all-target checkが成功した。「3D折り・紙厚・衝突」を64%、全体を48.40%（表示48.4%）へ更新した
- SIM-010候補の認証済み層順を専用3D viewerへ接続した。crossed overlap cellのexact境界をnative内でbounded world polygonへ変換し、cell hash・bottom-to-top FaceId・境界だけをDTO化してproof objectは非公開に保つ。日英panelはisometric exploded stack、cell切替、front/back・top/bottom、face hover/focus/click選択を提供し、project/revision/selected line/pose generation変化でselectionごと失効する。native、parser 3件、DOM 7件、build、lintが成功した。「3D折り・紙厚・衝突」を61%、全体を47.89%（表示47.9%）へ更新した
- SIM-010正厚対応の狭いnative certificateとして、2三角形face・1hinge・要求角90度以下を対象に、source 0度から各sampleの認証済みouter shell/static classifierとendpoint proofを同一pose・厚さへ束縛した。37度・0.1 mmでcontinuous certificateとsafe-stop 37度を発行し、equal-angle別pose ABAと厚さ1 ULP driftを拒否する。UI/transactionへ未接続なのでこの項目単独では完成率を変更していない
- IO-008の認証済みFOLD 3D全frameをinstruction timelineへ一括追加する利用者経路を接続した。全frameが同一topology、parentがindex順のlinear chain、rigid pose、current projectと完全一致するM/V hinge vector・fingerprint・fixed faceを持つ場合だけnative stageを発行する。日英UIの明示確認後、既定1,000 msの全frame stepを`AppendInstructionSteps` 1回でatomic追加し、geometryを変えず既存Undo/Redoで一括復元する。branching・非互換・stale・cancelはfail-closedにした。all-target check、frontend 4件、build、lintが成功した。「入出力・互換性」を97%、全体を47.55%（表示47.6%）へ更新した
- IO-008の正厚外殻unionを認証済みtreeの複数hingeへ拡張した。full exact treeから指定hingeのindex・incident triangular faces・material endpoint identityをissuer/poseへ束縛し、最大16 hingeのpackageを作る。rail interiorは非交差、共有endpointは同一material VertexIdとbit-exact座標の場合だけ許容し、consumerはjunctionを含む外殻をwatertight incidence・finite nonzero volume・保守的self-intersectionへ通す。3面2hingeの独立STL/GLB reader E2Eが成功した。「入出力・互換性」を96%、全体を47.50%（表示47.5%）へ更新した
- IO-008の生成GLBをKhronos公式`gltf-validator`固定versionでCI受入する。production APIからstatic・textured・animated GLBを毎回生成し、embedded resources込みerror/warning/info 0を要求する。導入時に検出したanimation morph POSITION accessorのmin/max欠落も修正した。外部GUI実機確認の代替とは扱わず別残件に保つ
...
- IO-008のtexture asset認証基盤をproject正本へ接続した。PNG/JPEGだけをmagic byteとmedia typeの一致まで検証するbounded registryを`.ori2`とexpanded folderの保存・読込・復旧へ保持し、asset単体16 MiB、合計32 MiB、64件の上限、nil・重複・参照欠損をfail-closedにした。boundedな未参照assetはUndo/Redoが旧textureを復元するため保持する。静的GLB captureは同じrevisionのproject registryからfront textureだけを解決し、UV付きPBR baseColorTextureへ渡す。back textureと紙厚付きtextureは現行の単一material/primitive契約で黙って損失させず拒否する。`ori-formats` 268件、desktop mesh export 13件、format検査が成功した。assetを登録・選択する利用者UIは未接続なので完成率は変更していない
...
- IO-008の内部形式基盤として、同一index topologyを持つ2〜256個のmesh frameとstrict増加timeを受理し、glTF 2.0のSTEP animation、morph target POSITION delta、全frame weight keyへ変換するGLB APIを追加した。frame topology不一致、非有限・非増加time、頂点・index・color差異、64 MiB超過をfail-closedにし、独立`gltf` readerでtime、weight、全morph deltaを再読込した。`ori-formats` 265件が成功した。折り手順timeline・desktop export UIへ未接続で利用者経路は増えていないため、IO-008の部分実装と全体完成率46.4%は維持する
- 認証済みinstruction timelineをGLB animationへ変換するnative preview/cancel commandを追加した。project instance・ID・revision・fold fingerprint・timeline全内容をcaptureと応答前に再検証し、declarative/foreign fingerprintを拒否する。planar初期frameと各stepの完全hinge poseをnative treeでsolveし、duration累積timeと同一topology meshをGLB immutable stageへ保持する。bytes・frameはWebViewへ渡さず、heavy-worker gate、stale generation、strict requestを回帰した。保存commandとfrontend UIが未接続なので完成率は変更していない
- instruction animationのnative immutable stageを既存のnative save dialog、filename正規化、no-follow原子的保存へ接続した。dialog前後・書込直前にproject instance・ID・revision・fingerprint・timeline・stage generationを再認証し、cancelはstageを保持してretry可能、成功時だけconsumeする。hingeあり初期frameは最初のstepのfixed faceを必須としてzero-angle poseをsolveし、2-hinge成功とfixed face欠損拒否を追加した。native 8件と全target checkが成功した。frontendにはstrict preview/save/cancel clientとmetadata validatorも追加したが、利用者UIは未接続なので完成率は変更していない
- IO-008の埋込texture形式基盤として、PNG/JPEG bytes、UV、PBR `baseColorTexture`、`TEXCOORD_0`、mime付きimage bufferView、4-byte paddingを追加した。16 MiB hard limit、payload signature、有限UV、頂点数一致を検証し、独立`gltf` readerでUV・mime・埋込bytesを再確認する。`ori-formats` 267件が成功した。project texture assetとdesktop exportへ未接続なので完成率は変更していない
- desktop静的GLBへtextureを接続する前段として、WebViewから構築できないnative `ResolvedStaticMeshTexture` stageを追加した。project instance・ID・revision、paperのtexture `AssetId`、front side、GLB限定、有限UV・頂点数一致を再検証し、材料座標から決定的なfront UVを生成する。現行projectにはasset IDに対応するnative bytes repositoryがなく、back textureには別primitive/materialが必要なため、推測やbrowser bytesを採用せず未接続へ閉じた。desktop lib checkは成功した。利用者経路は増えていないため完成率は変更していない
...
- SIM-010の読取専用workerへ、層別材料segmentからtarget平面配置を自動生成するbounded builderを追加した。source edgeと候補creaseの全pair交点を共有頂点へ正規化し、source vertex、source edgeごとの一つのidentity、外周分割を反映したpaper boundary cycleを保存する。新規IDはproject/revision/canonical geometryから再現可能に導出し、signed zero、入力順序・方向、欠損boundary、ID衝突、共線重複、duplicate source座標、無効crease、carrier・pair・頂点・edge上限をfail-closedにした。生成、face-lineage proof、stacked-fold geometry-delta proofを一つの所有packageへ統合し、証明済みtargetをnative material-tree kinematicsへ再admitした。current source poseのissuer、既存角度を継承した初期poseの全descendant transform一致を検証し、全新規crease subdivisionを集合hingeとして要求角へ置換したendpoint poseまでsolveする。endpointは同じnative model/poseと紙厚で公開静的衝突診断を全pair実行し、separated/touching/allowed/penetrating/indeterminate件数とblocking holdをDTOへ追加した。bit-exact 180度endpointではtarget topology・local conditionを再構築してglobal flat-foldability solverを実行し、Possibleなtarget layer-orderのmaterial face・overlap cell件数だけもDTOへ写す。対角線crossの自動生成からlineage・delta証明まで、外周中点分割、単一対角線targetのtree admission・0度初期pose・90度endpoint、要求角境界、重複・上限を回帰し、`ori-core` 271件、desktop read 2件、全target checkを通した。候補geometry/model/pose/proof/layer certificate自体は渡さずmutation authorityもfalseのままで、cycleを含む一般target kinematics、continuous collisionとsafe-stop、非180度target layer order、timeline、原子的command、UIが未実装なのでSIM-010状態と完成率46.4%は変更していない
- cycleを含む一般target kinematicsの前段として、material hinge graphを有限上限下で監査し、canonical Kruskal partitionによりspanning hingeとclosure hingeを保存順序非依存で分離する観測専用`MaterialHingeGraphAudit`を追加した。caller supplied全face transformについて全hingeのaxis両端像とMountain/Valley符号込みrelative rotationを検証するread-only closure certificateも追加し、不完全・重複face、角度欠損、axis drift、closure不一致を拒否する。SIM-010のproved target geometryはopaque graph audit packageへ束縛し、cycleを捨てずcanonical closure IDsと厳格failure classを後続certificateへtransportできる。connected tree/cycle、disconnected、duplicate、resource超過を回帰し、pose solveまたはmutation authorityを発行しない。`ori-kinematics` unit 15件、`ori-core` 272+6件は成功し、Windows App Controlが別integration executableをOS error 4551で阻止した。一般cycle candidate transform生成とclosure certificate接続が残るため完成率は変更していない
- nativeの折り重ね読取commandへstrict frontend client境界を追加した。project/revisionを再照合し、binding、cell SHA-256、face UUID、材料segment、topology proof全件数、endpoint collision全pair合計とblocking hold、work retained件数、180度layer-order整合、read-only authority falseを完全検証する。stale、未知enum、非有限・退化入力、nested DTO欠損・矛盾をfail-closedにし、専用Node 3件とproduction buildを通した。まだcontroller/UIへ接続せず、利用者経路ではないため完成率は変更していない
- 折り重ね読取の非公開frontend coordinatorを追加し、strict normalizerを通ったdetached resultだけをcurrent authorityへ公開する。開始前・完了時authority照合、replacement、invalidate、dispose、stale完了抑止、observer reentry所有権、transport例外の固定category化を5件で回帰し、型検査とlintを通した。利用者UIへ未接続なので完成率は変更していない

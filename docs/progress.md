...
**全体完成率: 約54.5%（2026-07-20、暫定の重み付き概算）**
...
下表の「全体への寄与」は「全体比率 × 現在の領域進捗」である。直前値53.46%に対し、限定正厚SIM-010を11 triangular faces・10 hingesへ拡張して「3D折り・紙厚・衝突」を92%から94%へ、全11種の幾何制約作成UIで「数式・幾何制約」を22%から30%へ更新した。差分は1.06ポイント（17% × 2ポイント + 9% × 8ポイント）で、54.52%を小数第1位へ丸めて表示している。実際のGitHub Release公開、SIM-010の一般姿勢・12face以上の正厚・cycle mutation・dense/証明困難な経路、幾何制約solver、一般経路探索は未完了である。入力値自体が概数なので、54.52%は追跡用の計算値であって測定誤差のない精密値ではない。
...
| 数式・幾何制約 | 9% | 30% | 2.70% | 数式入力、11種制約の保存・履歴・一覧・削除・原因表示に加え、全11種のtyped作成UIを接続。式駆動更新、拘束solverを残す |

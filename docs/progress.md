...
- SIM-010の読取専用workerへ、層別材料segmentからtarget平面配置を自動生成するbounded builderを追加した。source edgeと候補creaseの全pair交点を共有頂点へ正規化し、source vertex、source edgeごとの一つのidentity、外周分割を反映したpaper boundary cycleを保存する。新規IDはproject/revision/canonical geometryから再現可能に導出し、signed zero、入力順序・方向、欠損boundary、ID衝突、共線重複、duplicate source座標、無効crease、carrier・pair・頂点・edge上限をfail-closedにした。生成、face-lineage proof、stacked-fold geometry-delta proofを一つの所有packageへ統合し、証明済みtargetをnative material-tree kinematicsへ再admitした。current source poseのissuer、既存角度を継承した初期poseの全descendant transform一致を検証し、全新規crease subdivisionを集合hingeとして要求角へ置換したendpoint poseまでsolveする。target fingerprint、material face・hingeを含む有限件数だけをDTOへ写す。対角線crossの自動生成からlineage・delta証明まで、外周中点分割、単一対角線targetのtree admission・0度初期pose・90度endpoint、重複・上限を回帰し、`ori-core` 271件とdesktop全target checkを通した。候補geometry/model/pose/proof自体はWebViewへ渡さずmutation authorityもfalseのままで、cycleを含む一般target kinematics、live commit再認証、連続衝突、target layer order、timeline、原子的command、UIが未実装なのでSIM-010状態と完成率46.4%は変更していない

...
| 数式・幾何制約 | 9% | 55% | 4.95% | 回転軌道とポインター半直線の距離停留点を解析的に列挙し、端点・曖昧解・数値限界を決定的に分類 |
| 3D折り・紙厚・衝突 | 17% | 41% | 6.97% | 単一ヒンジの移動面を掴む物理ドラッグを、pointerup時の最終目標1要求だけで安全停止runnerへ接続 |
| 折り可能性・経路探索 | 18% | 8% | 1.44% | 2種類のドラッグ中の未確認目標を描画姿勢から分離し、runnerが認定した角度だけを3Dへ適用 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 59% | 2.95% | 両OS CI、実ファイル回帰に加え、物理把持・入力境界を含むfrontend 410件、Rust 302件を検証 |
...
- frontend単体・性能テスト410件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むRustワークスペーステスト302件
...
1. `physical_grab_v2`を木構造の選択ヒンジと従属面群へ拡張し、複数ヒンジ姿勢vectorの連続検証境界を設計する

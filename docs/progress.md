...
| 3D折り・紙厚・衝突 | 17% | 23% | 3.91% | 固定面を青枠、選択ヒンジから動く従属面群を橙枠で表示し、固定面変更後も動作側を再計算 |
...
| 多言語・設定・配布・QA | 5% | 46% | 2.30% | 両OS CI、実ファイル回帰に加え、固定面reroot・従属面反転・左右山谷を含むfrontend 157件を検証 |
...
- frontend単体・性能テスト157件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むori-coreテスト95件
...
1. 3D上のヒンジ・面の直接ピッキングを選択状態と固定面操作へ接続する

...
| 3D折り・紙厚・衝突 | 17% | 19% | 3.23% | 個別ヒンジ角度と選択状態を単一・木・閉路プレビューへ接続し、再構築なしの姿勢・material更新を実装 |
...
1. 固定面を選択できるようにし、選択面を基準とした木構造の親子関係と姿勢を再計算する
2. 選択ヒンジの従属面を2D・3Dで強調し、3D上の直接ピッキングを選択状態へ接続する

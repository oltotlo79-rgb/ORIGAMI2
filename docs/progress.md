...
| 3D折り・紙厚・衝突 | 17% | 24% | 4.08% | 3D上のヒンジクリックを選択同期へ、面クリックを固定面変更へ接続し、強調と姿勢を更新 |
...
| 多言語・設定・配布・QA | 5% | 47% | 2.35% | 両OS CI、実ファイル回帰に加え、Raycaster優先順位・空白・重複入力を含むfrontend 161件を検証 |
...
- frontend単体・性能テスト161件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むori-coreテスト95件
...
1. 自己衝突・紙同士の貫通を検出する広域判定を追加し、折り可能性と経路探索へ接続する

...
| 3D折り・紙厚・衝突 | 17% | 37% | 6.29% | 現在姿勢の狭域判定に加え、単一ヒンジの連続経路runnerを3D表示へ接続し、未確認角を描画せず衝突・未確定区間への進行を停止 |
| 折り可能性・経路探索 | 18% | 6% | 1.08% | 単一ヒンジの線形角度経路を1描画フレーム1作業単位で実行し、指定角と実表示角を分離して結果を表示 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 56% | 2.80% | 両OS CI、実ファイル回帰に加え、runnerの差替え・取消・再入境界と経路状態表示を含むfrontend 292件、Rust 302件を検証 |
...
- frontend単体・性能テスト292件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むRustワークスペーステスト302件
...
1. 単一折りの停止結果へ遮断面、危険・未確定角区間、停止角、原因の詳細表示を接続し、3Dドラッグ操作を同じ検証経路へ通す

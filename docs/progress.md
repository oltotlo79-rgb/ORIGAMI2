...
- 衝突分類のwatertight姿勢へ向けた第一checkpointとして、`ori-collision`内部に資源制限付き`rational_cayley_local_rotation_v1`を追加した。finite binary64の軸・角度をbit-exact有理数へ変換し、0度、符号付き180度、軸長二乗が有理数平方となる90度のexact枝と一般角のCayley回転を構成する。Machin π、外向きTaylor区間、有理平方根区間により、実現角誤差が入力角度の隣接binary64間隔の4分の1未満であることを証明し、`RᵀR = I`、`det R = 1`、軸方向と両端点の不動を完全有理算術で再検証する。subnormal、90度の前後1 ULP、179度、180度直前、最大4096-bit精度、符号・軸反転、決定論的property行列、各資源上限の境界を専用16件で回帰した。issuer-bound tree合成、全共有featureのwatertight一致、renderer姿勢との包含および衝突safe setへの接続は未実装なので、折り重ねUIへは進まず完成率も36.9%のままとする

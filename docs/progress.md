...
| 3D折り・紙厚・衝突 | 17% | 35% | 5.95% | 現在姿勢の広域・三角柱SAT・問題面輪郭に加え、中央面基準の共有ヒンジ許容接触とヒンジ外衝突を分離 |
...
| 多言語・設定・配布・QA | 5% | 54% | 2.70% | 両OS CI、実ファイル回帰に加え、共有ヒンジの真正性・通常角・全対走査を含むfrontend 226件を検証 |
...
- frontend単体・性能テスト226件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むori-coreテスト95件
...
1. 現在角から目標角までの連続衝突判定と安全停止時刻を実装し、折り可能性へ接続する

...
| プロジェクト・保存・履歴 | 8% | 53% | 4.24% | revision、順序保持Undo/Redo、紙設定と境界形状の履歴、ネイティブ`.ori2`読込・保存、新規作成を実装 |
| 2D展開図エディター | 15% | 27% | 4.05% | 基本編集、計測、任意多角形用紙の境界編集、グリッド・頂点・辺・中点スナップを実装 |
| 数式・幾何制約 | 9% | 27% | 2.43% | 完全な紙境界検証、矩形生成、比例変換、境界編集とスナップの異常値防御を実装 |
...
| 多言語・設定・配布・QA | 5% | 15% | 0.75% | 両OS CI、正式形式のアプリアイコン、macOS bundle検証、スナップ単体テストを追加 |
...
- 辺・中点スナップ時に通常辺を原子的に分割し、接続済み頂点を生成する編集操作
...
3. 通常辺の原子的分割を実装し、辺・中点スナップによる頂点追加と接続する
4. 交点・水平・垂直・平行・角度スナップへ拡張する

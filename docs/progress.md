...
- frontend単体・性能テスト139件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むori-coreテスト95件
...
- 複数折りでは誤った多関節回転を行わず、全材料面と全ヒンジを静的3D表示し「平面確認段階」と明示する段階的consumer対応
...
1. 複数ヒンジ用の折り状態、基準面、親子変換、対応可能なhinge graph分類を定義する
2. 木構造のhinge graphから、各面へ剛体変換を伝播する複数折り3D運動を実装する
3. 閉路を含むhinge graphを誤って動かさない拘束診断と、将来の閉路ソルバー境界を実装する
4. ネイティブ`.ori2`のダイアログ以外を自動E2E化し、Windows/macOS実機でダイアログ操作を回帰試験する
5. macOSのDock／OS終了時の未保存保護とクラッシュ時自動復旧を実装する

...
**全体完成率: 約27.7%（2026-07-18、暫定の重み付き概算）**
...
下表の「全体への寄与」は「全体比率 × 現在の領域進捗」である。2026-07-18の第三者監査（対象`013ba08`と当時の作業ツリー）を現在コードと再照合した26.44%を基準に、単一ヒンジ補正候補の解析専用UIで0.36ポイント、端末内redacted diagnosticsの閲覧・同一内容手動保存UIで0.50ポイント、川崎・前川の局所必要条件検証UIで0.36ポイントを加えた算術結果27.66%を小数第1位へ丸めて表示している。入力値自体が概数なので、27.66%は追跡用の計算値であって測定誤差のない精密値ではない。
...
| プロジェクト・保存・履歴 | 8% | 40% | 3.20% | 原子的編集、Undo/Redo、`.ori2`保存は実装。履歴永続化・自動保存・クラッシュ復旧は未着手 |
...
| 多言語・設定・配布・QA | 5% | 40% | 2.00% | frontend 746件・Windows Rust 340件、Windows/macOS CI、環境・作品情報を含まないredacted diagnosticsの端末内保存・正確な内容確認・同一JSON手動保存を実装。i18n、設定、更新、GitHub Releases配布を残す |
...
- 現在の3D姿勢を折り手順へ登録し、編集・`.ori2`保存・読込・再生する最初のタイムライン垂直スライス
...
1. 現在の3D姿勢、複数ヒンジ角、固定面を手動stepとして登録し、編集・`.ori2`保存・読込・再生できるタイムラインを実装する
2. MUST 86件のstatus表を各checkpointで維持する
3. 折り手順の次にSVG/FOLD/PDF、履歴永続化・復旧、i18n、単位、レイヤーの未着手MUSTをbreadth-firstで進める

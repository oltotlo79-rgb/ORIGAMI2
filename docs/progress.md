...
**全体完成率: 約22.0%（2026-07-17）**

完成率は画面数ではなく、要件定義書のMUST 86件、FUTURE 14件、品質検証、両OS配布を含む総工数の概算である。研究要素の結果によって見積もりを更新する。

下表の「全体への寄与」は「全体比率 × 現在の領域進捗」であり、合計22.01%を小数第1位へ丸めて全体完成率としている。

## 重み付け

| 領域 | 全体比率 | 現在の領域進捗 | 全体への寄与 | 状態 |
|---|---:|---:|---:|---|
| 要件・基本設計・技術検証 | 5% | 62% | 3.10% | 面抽出・平面トポロジーとmacOS終了・復旧保護まで設計を文書化 |
| プロジェクト・保存・履歴 | 8% | 60% | 4.80% | 原子的編集とUndo/Redo、ネイティブ`.ori2`実ファイル操作、全OSの原子的上書きと失敗時保護を実装 |
| 2D展開図エディター | 15% | 46% | 6.90% | 基本編集、任意多角形用紙、交点接続、9種のスナップ、境界一面の安定ID抽出基盤を実装 |
| 数式・幾何制約 | 9% | 45% | 4.05% | 紙境界検証と編集幾何に加え、filtered orientationと厳密な多角形符号付き面積を実装 |
...
| 多言語・設定・配布・QA | 5% | 35% | 1.75% | 両OS CI、macOS bundle、実ファイル回帰、候補ファズ、1万要素索引を追加 |
...
- 未対応の山折り・谷折り・切断線を誤って一面へ畳み込まず、3D移行を明示的に遮断するstrict診断
...
1. 折り線・切断線を含むhalf-edge rotation systemとwalk列挙を実装する
2. ネイティブ`.ori2`のダイアログ以外を自動E2E化し、Windows/macOS実機でダイアログ操作を回帰試験する
3. macOSのDock／OS終了時の未保存保護とクラッシュ時自動復旧を実装する
4. Windows/macOSの基準PCで1万辺の実測値を記録し、性能合格値を確定する

...
**全体完成率: 約20.8%（2026-07-16）**

完成率は画面数ではなく、要件定義書のMUST 86件、FUTURE 14件、品質検証、両OS配布を含む総工数の概算である。研究要素の結果によって見積もりを更新する。

下表の「全体への寄与」は「全体比率 × 現在の領域進捗」であり、合計20.79%を小数第1位へ丸めて全体完成率としている。

## 重み付け

| 領域 | 全体比率 | 現在の領域進捗 | 全体への寄与 | 状態 |
|---|---:|---:|---:|---|
| 要件・基本設計・技術検証 | 5% | 58% | 2.90% | 要件・基本設計、交点クラスタ数値規則、実データCanvas性能計測と固定描画層を文書化 |
| プロジェクト・保存・履歴 | 8% | 57% | 4.56% | revision、配列順を保つ原子的編集とUndo/Redo、紙設定と境界形状の履歴、最大64辺の交点一括接続、ネイティブ`.ori2`読込・保存、新規作成を実装 |
| 2D展開図エディター | 15% | 45% | 6.75% | 基本編集、計測、任意多角形用紙、交点接続、9種のスナップ、1万辺のbatch描画と局所選択を実装 |
...
| 多言語・設定・配布・QA | 5% | 32% | 1.60% | 両OS CI、macOS bundle、候補ファズ、1万要素索引、3万件indexed差分試験を追加 |
...
- frontend単体・性能テスト121件と、X交点・通常/輪郭T字・最大64辺クラスタ編集を含むori-coreテスト88件
...
1. ネイティブ`.ori2`のダイアログ以外を自動E2E化し、Windows/macOS実機でダイアログ操作を回帰試験する
2. macOSのDock／OS終了時の未保存保護とクラッシュ時自動復旧方針を確定する
3. Windows/macOSの基準PCで1万辺の実測値を記録し、性能合格値を確定する
4. 任意の有効な展開図から面を抽出し、3D折り計算へ渡すhalf-edge構造を設計する

...
| 2D展開図エディター | 15% | 39% | 5.85% | 基本編集、計測、任意多角形用紙の境界編集、補助線、輪郭T字を含む交点接続と9種のスナップを実装 |
| 数式・幾何制約 | 9% | 41% | 3.69% | 完全な紙境界検証、矩形生成、比例変換、辺・X交点・通常/輪郭T字交点分割、方向・平行・角度スナップの異常値防御を実装 |
...
| 多言語・設定・配布・QA | 5% | 27% | 1.35% | 両OS CI、正式形式のアプリアイコン、macOS bundle検証、平行・角度各10万件ファズと輪郭T字の原子性テストを追加 |
...
- スナップ単体・性能テスト74件と、X交点・通常/輪郭T字交点編集を含むori-coreテスト72件
...
3. 3本以上が同一点へ集まる交点を1回のUndoで復元できる一括接続へ一般化する
4. 10,000本データをRustからUIへ転送し、実Canvasの描画FPSを計測する

...
**全体完成率: 約56.8%（2026-07-20、暫定の重み付き概算）**
...
下表の「全体への寄与」は「全体比率 × 現在の領域進捗」である。直前値54.52%に対し、11種の幾何制約を扱う有界solverを頂点・辺駆動preview、拘束rank・自由度・条件推定、原子的Apply、Undo/Redoへ接続して「数式・幾何制約」を30%から55%へ更新した。差分は2.25ポイント（9% × 25ポイント）で、56.77%を小数第1位へ丸めて表示している。実際のGitHub Release公開、SIM-010の一般姿勢・12face以上の正厚・cycle mutation・dense/証明困難な経路、式駆動更新、一般経路探索は未完了である。入力値自体が概数なので、56.77%は追跡用の計算値であって測定誤差のない精密値ではない。
...
| 3D折り・紙厚・衝突 | 17% | 94% | 15.98% | 木構造多ヒンジ姿勢・紙厚・衝突・固定面・物理把持、高精度静的診断に加え、折り重ねを厚さ0単一hinge・同軸multi-hinge・adaptive sparse非同軸2〜64hinge、限定正厚1〜10hingeでpreview・原子的apply・Undo/Redoへ接続。候補/current層順3D viewerも実装。一般正厚・cycle mutation・dense/証明困難な経路を残す |

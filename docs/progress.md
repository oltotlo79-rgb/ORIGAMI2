...
| 3D折り・紙厚・衝突 | 17% | 33% | 5.61% | 広域・三角柱SAT・問題面輪郭に加え、モデル単位の静的三角形分割を角度フレーム間で再利用 |
...
| 多言語・設定・配布・QA | 5% | 52% | 2.60% | 両OS CI、実ファイル回帰に加え、prepared衝突解析の互換性・上限・入力変異を含むfrontend 195件を検証 |
...
- frontend単体・性能テスト195件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むori-coreテスト95件

...
**全体完成率: 約30.6%（2026-07-18、暫定の重み付き概算）**
...
下表の「全体への寄与」は「全体比率 × 現在の領域進捗」である。2026-07-18の第三者監査（対象`013ba08`と当時の作業ツリー）を現在コードと再照合した26.44%を基準に、単一ヒンジ補正候補の解析専用UIで0.36ポイント、端末内redacted diagnosticsの閲覧・同一内容手動保存UIで0.50ポイント、川崎・前川の局所必要条件検証UIで0.36ポイント、折り手順の作成・保存・段階再生で1.40ポイント、`.ori2`の手順格納範囲拡大で0.10ポイント、FOLD 2D展開図取込で0.70ポイント、SVG静的直線展開図の取込・線種確認・原子的適用で0.70ポイントを加えた算術結果30.56%を小数第1位へ丸めて表示している。SVG取込による0.70ポイントは「入出力・互換性」の領域進捗を30%から44%へ更新した差分（全体比率5% × 14ポイント）であり、FOLD/SVG/PDF等の出力、FOLDの3D・複数frame対応は計上していない。入力値自体が概数なので、30.56%は追跡用の計算値であって測定誤差のない精密値ではない。

## 重み付け

| 領域 | 全体比率 | 現在の領域進捗 | 全体への寄与 | 状態 |
|---|---:|---:|---:|---|
| 要件・基本設計・技術検証 | 5% | 70% | 3.50% | 要件定義・設計文書・技術検証は充実。未確定の物理厚さ・層順規則を残す |
...
| 入出力・互換性 | 5% | 44% | 2.20% | `.ori2`に加え、FOLD 1/1.1/1.2の2D `creasePattern`とSVG 1.1/2静的直線subsetの取込、縮尺・線種・外周・警告確認、原子的な新規未保存project化を実装。FOLDの3D・複数frame、各形式の出力を残す |
...
- SVG確認画面は実DOM試験を追加し、Tab/Shift+Tabと外部focusのtrap、IME中Escape、設定変更時の外周検証解除、失敗時dialog保持をcomponent eventで確認する。既存のsource契約試験と合わせ、frontendは793件を実行する
- IO-004とIO-005を実装済みへ更新。MUST 86件の現在集計は実装済み25・部分実装29・未着手32
...
- FOLD/SVG書出しを、共通の情報損失preview、revision固定stage、native原子保存まで含む利用者経路として接続する作業
...
1. FOLD/SVG出力を、情報損失preview、警告確認、原子的なnative保存まで一つの利用者経路として実装する
2. 続いて正確な展開図PDF/DXF出力、手順画像・折り図PDF出力を利用者経路単位で追加する

...
**全体完成率: 約27.0%（2026-07-17）**

完成率は画面数ではなく、要件定義書のMUST 86件、FUTURE 14件、品質検証、両OS配布を含む総工数の概算である。研究要素の結果によって見積もりを更新する。

下表の「全体への寄与」は「全体比率 × 現在の領域進捗」であり、合計27.00%を小数第1位へ丸めて全体完成率としている。

## 重み付け

| 領域 | 全体比率 | 現在の領域進捗 | 全体への寄与 | 状態 |
|---|---:|---:|---:|---|
| 要件・基本設計・技術検証 | 5% | 67% | 3.35% | hinge graphの木・閉路分類に加え、完全ID検証付き個別ヒンジ角度APIと安全なconsumer境界まで実装検証 |
...
| 3D折り・紙厚・衝突 | 17% | 18% | 3.06% | 木構造の累積剛体変換を個別ヒンジ角度へ拡張し、スライダー・数値入力から軽量姿勢更新へ接続 |
...
| 多言語・設定・配布・QA | 5% | 44% | 2.20% | 両OS CI、実ファイル回帰に加え、個別角度の完全ID検証・プロジェクト隔離を含むfrontend 150件を検証 |
...
- frontend単体・性能テスト150件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むori-coreテスト95件
...
1. ヒンジ選択を2D展開図と3D表示で同期し、対象の辺・ヒンジ・従属面を強調する
2. 固定面を選択できるようにし、選択面を基準とした木構造の親子関係と姿勢を再計算する
3. 自己衝突・紙同士の貫通を検出する広域判定を追加し、折り可能性と経路探索へ接続する
4. 変換行列の再利用と描画フレーム単位の角度更新により、大規模な面・ヒンジ操作を最適化する
5. 閉路ソルバー境界の詳細化、ネイティブ`.ori2`の自動E2E、Windows/macOS実機回帰を進める

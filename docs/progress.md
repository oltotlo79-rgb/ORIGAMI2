...
| 3D折り・紙厚・衝突 | 17% | 38% | 6.46% | 単一ヒンジの安全停止に、確認済み境界、衝突姿勢を含む探索区間、対象面と分類の詳細表示を接続 |
| 折り可能性・経路探索 | 18% | 7% | 1.26% | 経路順を保つ危険・未確定bracketと、開始姿勢未確認・開始点のみ確認・正区間確認済みを区別して表示 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 57% | 2.85% | 両OS CI、実ファイル回帰に加え、停止詳細の不変化・未知ID非露出・不正結果遮断を含むfrontend 300件、Rust 302件を検証 |
...
- frontend単体・性能テスト300件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むRustワークスペーステスト302件
...
1. 単一折りの3Dドラッグ操作を同じ連続検証経路へ通し、ドラッグ中も未確認角を描画しない入力制御を追加する

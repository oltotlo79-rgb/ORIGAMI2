...
| 3D折り・紙厚・衝突 | 17% | 36% | 6.12% | 現在姿勢の狭域判定に加え、単一ヒンジの保守的swept AABB、共有ヒンジ区間支持、数値保証境界を実装 |
| 折り可能性・経路探索 | 18% | 4% | 0.72% | 単一ヒンジの線形角度経路を時刻順に証明し、安全下限・危険/未確定区間を返す中核ジョブを実装 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 55% | 2.75% | 両OS CI、実ファイル回帰に加え、連続経路・微小角・薄紙境界を含むfrontend 268件、Rust 302件を検証 |
...
- frontend単体・性能テスト268件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むRustワークスペーステスト302件
...
- 3D診断を非隣接とヒンジ外の貫通・接触、モデル許容境界接触・領域内重なり、ヒンジ未解決、数値・方針不確定へ分離し、「中央面基準の現在姿勢のみ・連続運動は未検証」を明示
...
1. 単一折りの連続衝突ジョブをフレーム単位で実行し、指定角と証明済み表示角を分離して3Dプレビューへ安全停止を接続する
2. OQ-002の物理的な厚さoffset・層ずれ規則を確定し、中央面基準の近似分類と選択可能にする

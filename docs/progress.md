...
| 3D折り・紙厚・衝突 | 17% | 49% | 8.33% | 危険点判定と同じ2面行列・完全角度vector・bounded witnessを保持し、局所法線・分離距離を解析情報としてUIへ提示 |
| 折り可能性・経路探索 | 18% | 13% | 2.34% | 危険根拠をruntime request identityへ結合し、古い要求や再入更新の証拠を表示しない停止詳細境界を実装 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 70% | 3.50% | frontend 553件、決定論的2,500ジョブ差分、Windows/macOS Rust、macOS `.app`をCI検証 |
...
2. 複数の衝突三角柱pairから全体修正候補をboundedに生成し、自動適用せず、完全衝突再検証で安全性を確認できた候補だけをプレビュー可能にする

...
**全体完成率: 約50.8%（2026-07-20、暫定の重み付き概算）**
...
下表の「全体への寄与」は「全体比率 × 現在の領域進捗」である。直前値50.10%に対し、厚さ0sparse adaptive theoremを33〜64 hingesへ、限定正厚SIM-010を3 face・2 hingesへ拡張し、「3D折り・紙厚・衝突」を74%から78%へ更新した。差分は0.68ポイント（17% × 4ポイント）で、50.78%を小数第1位へ丸めて表示している。Blender、slicer、Web viewerのGUI実機受入、一般曲面・非三角面union、完全な印刷可能性保証、GitHub Releases正式配布、SIM-010の一般姿勢・一般正厚・dense/証明困難な経路、一般経路探索は未完了である。入力値自体が概数なので、50.78%は追跡用の計算値であって測定誤差のない精密値ではない。
...
| 3D折り・紙厚・衝突 | 17% | 78% | 13.26% | 木構造多ヒンジ姿勢・紙厚・衝突・固定面・物理把持、高精度静的診断に加え、折り重ねを厚さ0単一hinge・同軸multi-hinge・adaptive sparse非同軸2〜64hinge、限定正厚single/2-hingeでpreview・原子的apply・Undo/Redoへ接続。候補/current層順3D viewerも実装。一般正厚・dense/証明困難な経路を残す |

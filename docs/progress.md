...
**全体完成率: 約29.2%（2026-07-18、暫定の重み付き概算）**
...
| 入出力・互換性 | 5% | 16% | 0.80% | `.ori2`へ展開図、紙の見た目、折り手順を格納。SVG/FOLD/DXF/OBJ/STL/glTF/PDFは未着手 |
| 多言語・設定・配布・QA | 5% | 40% | 2.00% | frontend 763件・Windows Rust 384件、Windows/macOS CI、環境・作品情報を含まないredacted diagnosticsの端末内保存・正確な内容確認・同一JSON手動保存を実装。i18n、設定、更新、GitHub Releases配布を残す |
...
1. 外部の既存展開図を取り込めるよう、FOLD形式の読込とmapping確認UIを最初の入出力垂直スライスとして実装する
2. 続いてSVG読込、FOLD/SVG出力、手順画像・PDF出力を利用者経路単位で追加する

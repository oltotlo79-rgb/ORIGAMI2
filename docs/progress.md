...
**全体完成率: 約37.8%（2026-07-19、暫定の重み付き概算）**
...
下表の「全体への寄与」は「全体比率 × 現在の領域進捗」である。直前値37.64%に対し、LIN-003の白黒印刷でも識別できる5線種をCanvas、取込プレビュー、SVG書き出しへ一貫して接続し、「2D展開図エディター」を50%から51%へ更新した。差分0.15ポイント（全体比率15% × 1ポイント）を加えた37.79%を小数第1位へ丸めて表示している。privateな正厚三角柱交差核とexact E有限ヒンジ回廊証明は利用者経路へ未接続なので加算していない。native診断による操作の巻戻し・連続経路停止、正厚の本番証明、専用の層順3D viewer、SIM-010の折り重ね、一般経路探索、FOLDの3D・複数frame、3D完成形出力も計上していない。入力値自体が概数なので、37.79%は追跡用の計算値であって測定誤差のない精密値ではない。

...
- cycleを含む一般target kinematicsの前段として、material hinge graphを有限上限下で監査し、canonical Kruskal partitionによりspanning hingeとclosure hingeを保存順序非依存で分離する観測専用`MaterialHingeGraphAudit`を追加した。connected tree/cycle、disconnected、duplicate、resource超過を回帰し、closure hingeを捨てたtree solveまたはmutation authorityを発行しない。`ori-kinematics` unit 12件は成功し、Windows App Controlが別integration executableをOS error 4551で阻止した。一般cycle poseには全closure constraintの幾何再検証が残るため完成率は変更していない

...
| プロジェクト・保存・履歴 | 8% | 50% | 4.00% | revision、順序保持Undo/Redo、紙設定の単一状態管理、ネイティブ`.ori2`読込・保存、新規作成を実装 |
| 2D展開図エディター | 15% | 16% | 2.40% | 基本編集、可変寸法用紙、線の長さ・角度計測を実装 |
...
| 多言語・設定・配布・QA | 5% | 13% | 0.65% | 両OS CI、正式形式のアプリアイコン、macOS bundle検証を追加 |
...
- 長方形用紙と全頂点を比例変換し、Undo/Redo可能にするサイズ変更操作
...
3. 長方形用紙の幅・高さ変更を全頂点の比例変換、Undo/Redo、保存へ統合する

...
| 3D折り・紙厚・衝突 | 17% | 43% | 7.31% | 木構造の物理把持準備、重なり面pickingに加え、全材料面・全ヒンジ姿勢を検証後に一括適用する原子的シーン境界を実装 |
| 折り可能性・経路探索 | 18% | 9% | 1.62% | 木構造の選択1ヒンジについて、他ヒンジ固定の連続衝突ジョブ、motion context、所有権調停を実装。物理把持UIへの接続は未完了 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 64% | 3.20% | 両OS CI、実ファイル回帰に加え、木構造連続解析・所有権・原子的姿勢適用を含むfrontend 485件、Rust 302件を検証 |
...
- frontend単体・性能テスト485件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むRustワークスペーステスト302件
...
1. 実装済みの木構造選択1ヒンジ連続衝突解析、motion owner、原子的姿勢適用を3D物理把持UIへ接続し、他ヒンジを固定した認定済み角度だけを確定する

...
| プロジェクト・保存・履歴 | 8% | 56% | 4.48% | revision、配列順を保つ原子的編集とUndo/Redo、紙設定と境界形状の履歴、ネイティブ`.ori2`読込・保存、新規作成を実装 |
| 2D展開図エディター | 15% | 38% | 5.70% | 基本編集、計測、任意多角形用紙の境界編集、補助線、交点接続と9種のスナップを実装 |
| 数式・幾何制約 | 9% | 40% | 3.60% | 完全な紙境界検証、矩形生成、比例変換、辺・X交点・T字交点分割、方向・平行・角度スナップの異常値防御を実装 |
...
| 多言語・設定・配布・QA | 5% | 26% | 1.30% | 両OS CI、正式形式のアプリアイコン、macOS bundle検証、平行・角度各10万件ファズを含む幾何・スナップ・性能テストを追加 |
...
- スナップ単体・性能テスト71件と、X交点・T字交点編集を含むori-coreテスト67件
...
- 輪郭辺を含む交点接続と、3本以上が同一点へ集まる交点の原的一括接続設計
...
3. 輪郭辺とのT字交点を紙境界も同時更新する原子命令で接続する
4. 3本以上が同一点へ集まる交点を1回のUndoで復元できる一括接続へ一般化する
5. 10,000本データをRustからUIへ転送し、実Canvasの描画FPSを計測する

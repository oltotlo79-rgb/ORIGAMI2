...
- 3Dへ実際に表示された完全姿勢を、project instance・project ID・revision・固定面・全hinge角度とともにnative current-pose authorityへ適用するproduction commandを接続した。applyと診断の双方はJavaScript精度を失わない10進文字列のpose generationを返し、frontendは4項目のbinding完全一致時だけ結果を採用する。FoldPreviewが同じrenderで算出した実描画姿勢keyを親の観測姿勢keyと照合するため、新姿勢または移動中姿勢の最初のpaintから旧い緑表示を隠す。native apply・診断は常に1 transactionだけを直列実行し、待機要求は同一姿勢をまとめつつ最新の異なる姿勢だけへcoalesceする。greenの「横断貫通なし」はC certificate発行後だけ、証明済み横断は「安全認定不可」とし、証拠不足・資源上限・状態不整合は理由をバッジ本文にも表示する。確認中・姿勢確定待ちは`status` / `polite`、終端blockingだけは`alert` / `assertive`とし、実行失敗時は現表示姿勢を明示的に再試行できる。これは読み取り診断であり、操作を停止・巻戻したとは表示しない。bit-exactな紙厚`+0.0`・三角material faceの横断だけが現在の肯定範囲で、正厚・共有ヒンジ・polygon・連続経路は未対応である。利用者経路が増えたため3D領域を50%から52%、全体完成率を36.90%から37.24%へ更新した
...
- private actual-mm scanをnativeの公開静的衝突入口へblocking専用で接続した。従来のzero-thickness全pair走査と件数照合を先に完了し、bit-exactな紙厚`+0.0`の複数面だけで同じissuer/poseに束縛したCayley exact tree、measured envelope、dual-gate横断scanを実行する。証明済み横断がある場合だけ`ProvenTransversalPenetration`を返し、肯定0件・証拠不足・未対応形状は従来どおり安全証明を発行しない。旧classifierの判定値は新しい肯定errorの根拠に使わず、安全証明constructorは単一面zero-pair枝の一箇所のままとした。旧解析と新facadeはtriangle、pair、registry、境界関係、入力・保持clone・演算・GCD・allocation・出力の全累積予算を一つの呼出側上限から差し引き、Cayley三段のhard clamp・逐次残量・構造aggregate preflightを通す。旧exact snapshotは新exact姿勢の前に解放する。三段の構造予約は角V・中点V・三角形＋四角形fixtureの実測counter和を必ず覆い、予約値ちょうどの成功と1不足の事前拒否を機械的に固定した。Releaseでは公開integration 8件とlib 136件、blocking対象25件、全target・全feature check、Clippy `-D warnings`、fmtを通過した。角起点山谷Vは正逆source collection・全root・指定8角度で誤肯定0件、辺中点山山Vは135/179度だけ肯定1件、90/91/180度はblockingな証拠不足、`-0.0`と正厚は非昇格である。この公開入口checkpoint時点では正厚・有限ヒンジ・continuous collision・cell/layer transport・利用者向け理由表示・SIM-010が未完成だったため、MUST集計と完成率36.9%は変更しなかった
...
- exact tree `E`、測定元`E`をpointer identityで保持する`MeasuredBinary64AffineEnvelope`、`BoundMaterialTreePose`を同時に再認証するprivate actual-mm scanを実装した。対象はwhole material boundaryが3頂点のfaceに限定し、共有関係をexact boundaryの`VertexId`とhinge registryから内部導出する。dual gateは、canonical exact `E`のzero-width厳密横断と、同じissuer-bound poseの保存binary64 affine係数・rest頂点をactual-mm `BigRational`へ直接liftした実在triangleのzero-width厳密横断の双方が`ProvenPenetrating`であることに訂正した。measured envelopeはexact pointer/pose authorityとdirect-lift各点のradius内包含を再検証するだけで、per-face boxを共有点へweldしたり貫通幾何へ使ったりしない。独立box内に離間した実現姿勢が存在する監査反例を`Unresolved`へ固定し、共有頂点の正長relative-interior segment special caseもbit-exactに一致するzero-width共有点だけへ限定した。全canonical unordered face pairへrecordを作り、exact tree work、measured containment work、両zero-width predicateを一つの`WorkMeter`へ累積し、件数不一致・資源one-shortでは肯定済みpairを含めscan全体を原子的に返さない。bridgeのexact上限はresumeで観測できる12項目だけの型へ分離し、証明生成専用のprecision/guard/candidate 3項目を0固定して新規candidate発行を不可能にした。構造15項目とexact 12項目は全てexact/one-shortと独立hard ceiling直上を回帰する。400 mm角起点山谷Vは通常・逆source collection、全3 root、片側10度左右と両45/90/91/135/179/180度で`ProvenPenetrating` 0件、辺中点山山Vは同じsource/root軸で135/179度だけ外側1 pairを肯定し、90/91/180度を`Unresolved`へ固定した。bit-exactな紙厚`+0.0`だけを受け入れ、`-0.0`、正厚、非有限値、共有ヒンジ、非三角material faceは安全側へ閉じる。正厚証拠、有限ヒンジ、polygon、public proof、desktop certificate、production診断、rendererまたはSIM-010へは未接続なので、MUST集計32 / 27 / 28、オーナー指定の「衝突分類後に折り重ね」順序および完成率36.9%は変更しない

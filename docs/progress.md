...
| プロジェクト・保存・履歴 | 8% | 55% | 4.40% | revision、配列順を保つ原子的編集とUndo/Redo、紙設定と境界形状の履歴、ネイティブ`.ori2`読込・保存、新規作成を実装 |
| 2D展開図エディター | 15% | 31% | 4.65% | 基本編集、計測、任意多角形用紙の境界編集、補助線、5種のスナップを実装 |
| 数式・幾何制約 | 9% | 33% | 2.97% | 完全な紙境界検証、矩形生成、比例変換、辺・交点分割とスナップの異常値防御を実装 |
...
| 多言語・設定・配布・QA | 5% | 20% | 1.00% | 両OS CI、正式形式のアプリアイコン、macOS bundle検証、幾何・スナップ・性能テストを追加 |
...
- スナップ単体・性能テスト33件と、交点編集を含むori-coreテスト62件
...
- コミット`20ec83a`の交点スナップUIに対するWindows/macOS/frontend CI確認
- T字交点で既存端点を再利用し、相手側の辺だけを原子的に分割する編集操作
- 水平・垂直スナップと作図アンカー
...
3. T字交点で既存端点を再利用する原子的分割へ拡張する
4. 水平・垂直、平行、角度スナップを順に追加する

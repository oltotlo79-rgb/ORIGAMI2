...
**全体完成率: 約48.4%（2026-07-20、暫定の重み付き概算）**
...
下表の「全体への寄与」は「全体比率 × 現在の領域進捗」である。直前値47.89%に対し、SIM-010を限定正厚single hingeと厚さ0同軸multi-hingeへ拡張し、「3D折り・紙厚・衝突」を61%から64%へ更新した。差分は0.51ポイント（17% × 3ポイント）で、48.40%を小数第1位へ丸めて表示している。Blender、slicer、Web viewerのGUI実機受入、一般曲面・非三角面union、完全な印刷可能性保証、GitHub Releases正式配布、SIM-010の一般姿勢・一般正厚・非同軸multi-hinge経路、current全体層順viewer、一般経路探索は未完了である。入力値自体が概数なので、48.40%は追跡用の計算値であって測定誤差のない精密値ではない。
...
| 3D折り・紙厚・衝突 | 17% | 64% | 10.88% | 木構造多ヒンジの姿勢・紙厚・衝突・固定面・物理把持、高精度静的診断に加え、折り重ねを厚さ0単一hinge・同軸multi-hinge、限定正厚single hingeでpreview・原子的apply・Undo/Redo、候補層順3D viewerまで接続。正厚三角柱の一般分類、一般native連続停止、current全体層順表示、一般姿勢・一般正厚・非同軸multi-hinge折り重ねを残す |

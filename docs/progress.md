...
- 衝突分類のwatertight姿勢へ向け、privateな`rational_cayley_tree_pose_v1`を実装した。偽造不能な`BoundMaterialTreePose`から同一issuerとpose instance、fixed root、canonical face・hinge・完全角度列・全boundaryを再認証し、既存solverと同じBFSおよび山谷×左右の整数符号で全局所Cayley回転を完全有理合成する。全合成回転の`RᵀR = I`と`det R = 1`、全VertexId occurrence、共有ヒンジ両端・中点の完全一致、root変更時の単一global exact合同、source順不変、clone可の元poseと同角度再solve ABA・別issuerの区別をtree専用5件で固定した。全boundaryを一度だけ走査する認証済み辺索引を追加し、face・hinge・隣接・境界・索引・一意頂点・Machin/Taylor項・sqrt・演算・一時値を含む個別出力bit・分子と分母を合算した総storage bitの全上限を、実測値成功・one-short失敗で回帰した。増分前の総項数拒否とstorage課金は資源監査2件でも固定し、`ori-collision`のlib 58件を通過した。renderer誤差包含、deep-chain stress、厚さ0classifierへの接続とsafe set拡張は未実装なので、折り重ねUIへ進まず完成率も36.9%のままとする

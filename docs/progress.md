...
- SIM-010の読取専用workerへ、層別材料segmentからtarget平面配置を自動生成するbounded builderを追加した。source edgeと候補creaseの全pair交点を共有頂点へ正規化し、source vertex、source edgeごとの一つのidentity、外周分割を反映したpaper boundary cycleを保存する。共線重複、duplicate source座標、欠損endpoint、無効crease、carrier・pair・頂点・edge上限超過はfail-closedとした。生成直後に既存のface-lineage proofとstacked-fold geometry-delta proofを実行し、target fingerprintと有限件数だけをDTOへ写す。対角線crossの自動生成からlineage・delta証明まで、外周中点分割、重複・上限を回帰し、`ori-core` 270件とdesktop全target checkを通した。候補geometry/proof自体はWebViewへ渡さずmutation authorityもfalseのままで、live commit再認証、連続衝突、target layer order、timeline、原子的command、UIが未実装なのでSIM-010状態と完成率46.4%は変更していない

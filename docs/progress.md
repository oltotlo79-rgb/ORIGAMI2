...
| 3D折り・紙厚・衝突 | 17% | 39% | 6.63% | 単一ヒンジの安全停止へ、実表示角を始点とする上下ドラッグ目標をpointerup時の1要求だけで接続 |
| 折り可能性・経路探索 | 18% | 8% | 1.44% | ドラッグ中の未確認目標を描画姿勢から分離し、既存runnerが認定した角度だけを3Dへ適用 |
| 折り手順・PDF | 10% | 1% | 0.10% | タイムラインUI試作のみ |
| 入出力・互換性 | 5% | 16% | 0.80% | 安全制限付き`.ori2`、実パス読込・保存・再上書き・全OSの原子的置換を実装 |
| 多言語・設定・配布・QA | 5% | 58% | 2.90% | 両OS CI、実ファイル回帰に加え、ドラッグ入力境界28件を含むfrontend 328件、Rust 302件を検証 |
...
- frontend単体・性能テスト328件と、X交点・通常/輪郭T字・最大64辺クラスタ編集・複数折り面解析を含むRustワークスペーステスト302件
...
1. 折り面上の把持点とポインター投影から目標姿勢を求める`physical_grab_v2`を`vertical_parameter_v1`とは別契約で設計し、同じ連続検証runnerへ接続する

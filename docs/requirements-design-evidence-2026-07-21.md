...
`requirements-definition.md`のMUST 87件と`requirements-status.md`の正本表を照合した結果は、実装済み86件、部分実装1件、未着手0件である。部分実装はSIM-010であり、一般姿勢の複数層transport・正厚・多hinge連続経路とclosureという未完成境界を状態表に明記している。EDT-009は証明可能な7種の直接矛盾についてcanonicalかつ削除最小な原因制約IDを利用者へ表示し、証明できない一般制約を判定保留にするsoundな境界でMUSTを満たす。局所solverの非収束、rank不足、処理上限を矛盾またはMUSの証明とは扱わない。したがってSIM-010が残る間は初版MUST全体が完成したとは扱わない。

...
| EDT-009 | 部分実装 | `DirectConstraintConflictKindV1`のlegacy 21 variantをwire互換として維持し、10/11制約種を横断する有界binary64 exact-zero closure専用tagを2件追加して合計23 variantとした。実残差で`DirectConflict`へ昇格するsound allowlistは`DifferentFixedLengths`、`DifferentFixedAngles`、`HorizontalAndVertical`、`EqualLengthWithDifferentFixedLengths`、`LengthRatioWithIncompatibleFixedLengths`、`DifferentLengthRatios`、`DifferentFixedLengthsInEqualLengthComponent`、`ParallelWithPerpendicularOrientations`、`SameOrientationWithFixedNonParallelAngle`、`PerpendicularOrientationsWithFixedNonRightAngle`、`DifferentRotationalSymmetryAnglesWithFixedRadius`、`PositiveFixedLengthInBoundedZeroLengthClosure`、`ZeroLengthClosureReachesNondegenerateProvider`、`EqualLengthWithNonUnitRatioAndFixedLength`、`NonReciprocalLengthRatiosWithFixedLength`、`NonUnitLengthRatioCycleWithFixedLength`、`InconsistentLengthRatioGraphWithFixedLength`の合計17種である。回転familyは凍結sin/cosがbit-exactな90・180・270度のcardinal matrixを返す同一center/source/target役割の異なる2回転と、center-sourceまたはcenter-target実在辺上のconsistentな正有限`FixedLength`に限定する。各原因削除後は、固定長削除時の2回転collapseをsource patternと完全vertex registryを検証するprivate residual-only overlayで、片方の回転削除時のcardinal回転＋固定半径をgeometry-validなexact orbitで、それぞれ全production residualに対する独立SAT certificateとして再認証する。残る6 variantはcollapse、signed-zero、角度wrap、丸め、正規化residualの実反例があるため一般solverの証明なしに肯定せず、関係する制約IDを保持して`Unknown`へfail-closedとする。16制約以下ではこの17種だけをsoundなsubset oracleとして最大65,535回探索し、canonicalな最小基数proof coreを表示するが、各1件削除後の独立SAT certificateを再認証できた最小coreだけをsemantic MUSとして扱う。16制約超では肯定済み直接矛盾を維持したままsubset最小化未実行を明示する。zero closureと一般有向比率閉包は256制約までを独立のwork・storage上限下で調べ、制約数・work・storage・取消・deadlineの原因を区別して`Unknown`へ閉じる。desktop取消はproject instance・project・revision・request generationの完全一致時だけactive tokenへ伝達し、2秒のabsolute deadlineを持つ。ratio familyはexact-real積、逆向き除算、epsilon、初期座標を根拠にせず、solver共有のbinary64残差がゼロなら`Unknown`へ閉じる。zero closureと一般比率閉包のratio逆向きも正subnormal denominatorが正の比率乗算で+0へunderflowするSAT反例があるため使用しない。`Parallel`はzero伝播に使わずterminalに限定し、`FixedAngle`はcollapse・same-orientation・perpendicular時の実`atan2`到達classを共有残差で全拒否できる場合だけ肯定する。初期座標、solver収束、epsilon、近似長、別点・別辺を証拠にしない。legacy 6 variantを診断wireから削除したとは主張せず、`DirectConflict`へ昇格しない互換variantとして隔離する。全11種の一般充足可能性、完全な一般矛盾原因、一般最小不能部分集合は未完成である |

...
| EDT-003 | 部分実装 | 座標・角度補助に加え、選択頂点を始点として表示単位の正長と度単位の角度から終点を直接追加できる。終点と線を一つの原子的command・一回のUndoとして同時作成する経路を残す |

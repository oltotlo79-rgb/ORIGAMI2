...
| LIN-004 | 部分実装 | version固定のordered layer文書、implicit default layer、折り線edge assignment、作成・改名・並べ替え・削除・割当command、全geometry分割時の継承、Undo/Redo、`.ori2` required feature、通常open・復旧・strict IPC snapshotに加え、作成・改名・並べ替え・削除・選択折り線の割当を行う日英layer管理UIを実装。layer共通の表示・lock・透明度はLIN-005で実装済み。注釈・下絵object自体の作成・編集・描画を残す |
...
| EDT-004 | 部分実装 | 新規プロジェクトの紙幅・紙高で、小数・分数・平方根・π・四則演算・括弧を含む式をnative高精度評価して作成できる。座標、既存頂点、長さ、角度、既存用紙サイズの数式入力を残す |
...
| INS-008 | 部分実装 | 中割り・かぶせ・沈め折り、一直線の折り重ね、層選択説明を、複数操作・parameter・precondition・日英名を持つ名前付き宣言技法として作成・編集できる。未対応の物理操作は明示し、自動実行・project変更を行わない。技法からtimeline手順を生成・適用する利用者確認経路を残す |
...
| IO-008 | 部分実装 | OBJ/GLBをBlender・Web・他3Dアプリへ、STLを3Dプリンター向けworkflowへ受け渡せる静的出力を実装。折り手順を時間軸へ変換したanimation、material・texture、厚み付き閉solidと印刷可能性保証を残す |
| IO-009 | 部分実装 | 2D各形式に加え3D書き出し前にも、中央面meshのみ、厚み・材質・texture・animation・project意味情報を保持しないこと、STLの印刷適合性を保証しないことを形式別に表示し、明示確認をnativeでも強制する。animation等の将来形式への適用を残す |

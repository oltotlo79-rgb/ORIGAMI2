...
| SIM-008 | 部分実装 | topology snapshotへ元紙ID付きの決定的material componentを追加し、境界から境界へ達するCutを別componentへ分離する。Cut incidenceは左右faceを保持するがhinge adjacencyへ入れず、frontendも全face/component partitionを再検証して両片を静的3D表示する。切断禁止は従来どおり拒否する。closed cut loop、開いたseam、穴、分岐Cutを含む一般DCEL表現を残す |

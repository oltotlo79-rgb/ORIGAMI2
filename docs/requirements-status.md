...
| EDT-009 | 部分実装 | `DirectConstraintConflictKindV1`のlegacy 21 variantをwire互換として維持し、10/11制約種を横断する有界binary64 exact-zero closure専用tagを2件追加して合計23 variantとした。実残差で`DirectConflict`へ昇格するsound allowlistは`DifferentFixedLengths`、正の非退化providerを同一edgeへ持つ`HorizontalAndVertical`、`EqualLengthWithDifferentFixedLengths`、consistentな正の有限固定長2件と正の有限比率をsolverと同じ演算順で評価した残差が非ゼロまたは非有限となる`LengthRatioWithIncompatibleFixedLengths`、同じordered edge pairの異なる正有限比率2件とconsistentな正有限固定分母から得るbinary64積が異なるか非有限となる場合だけ3-IDで肯定する`DifferentLengthRatios`、`DifferentFixedLengthsInEqualLengthComponent`、direct `ParallelWithPerpendicularOrientations`、同じ2辺へ`Horizontal`×2または`Vertical`×2がありproductionのzero-cross全classをsolver共有residualが拒否する場合だけ肯定する`SameOrientationWithFixedNonParallelAngle`、同じ2辺の一方に`Horizontal`、他方に`Vertical`がありproductionの0・π/2・π・NaN全classをsolver共有residualが拒否する場合だけ肯定する`PerpendicularOrientationsWithFixedNonRightAngle`、`Horizontal`/`Vertical`のendpoint座標等価pathでforced-zero edgeを作り`EqualLength`双方向と`LengthRatio` denominator-zero→numerator-zeroだけを伝播して正有限固定長へ到達する`PositiveFixedLengthInBoundedZeroLengthClosure`、同じclosureがproductionのbinary64残差でcollapseを必ず拒否できる`PointOnLine`のline、`MirrorSymmetry`のaxis、`AngleBisector`の3辺、`Parallel`の2辺、`FixedAngle`の2辺へ到達する`ZeroLengthClosureReachesNondegenerateProvider`に加え、consistentな正有限固定長からsolverと同じbinary64演算順で導出した共有残差が非ゼロまたは非有限となる`EqualLengthWithNonUnitRatioAndFixedLength`、`NonReciprocalLengthRatiosWithFixedLength`、`NonUnitLengthRatioCycleWithFixedLength`、consistentな正有限固定長1件を独立rootとして正有限`LengthRatio`をdenominatorからnumeratorへだけ伝播し、同一root由来の2経路が同一edgeへ閉じた共有残差が非ゼロまたは非有限の場合だけ肯定する`InconsistentLengthRatioGraphWithFixedLength`の合計15種である。残る8 variantはcollapse、signed-zero、角度wrap、丸め、正規化residualの実反例があるため一般solverの証明なしに肯定せず、関係する制約IDを保持して`Unknown`へfail-closedとする。16制約以下ではこの15種だけをsoundなsubset oracleとして最大65,535回探索し、canonicalな最小基数proof coreを表示するが、各1件削除後の独立SAT certificateを伴うsemantic MUSとは扱わない。16制約超では肯定済み直接矛盾を維持したままsubset最小化未実行を明示する。zero closureと一般有向比率閉包は256制約までを独立のwork・storage上限下で調べ、制約数・work・storage・取消・deadlineの原因を区別して`Unknown`へ閉じる。desktop取消はproject instance・project・revision・request generationの完全一致時だけactive tokenへ伝達し、2秒のabsolute deadlineを持つ。`HorizontalAndVertical`の第三原因は同一edgeのcollapseを実solver residualで拒否する正の非退化providerでなければならず、各ratio familyはexact-real積、逆向き除算、epsilon、初期座標を根拠にせず、solver共有のbinary64残差がゼロなら`Unknown`へ閉じる。`DifferentLengthRatios`も2つの積が同じ有限binary64値へ丸まる場合は共有underflow zeroを含め`Unknown`へ閉じ、固定分母を欠く旧2-ID原因を生成しない。zero closureと一般比率閉包のratio逆向きも正subnormal denominatorが正の比率乗算で+0へunderflowするSAT反例があるため使用しない。`Parallel`はzero伝播に使わずterminalに限定し、`FixedAngle`はcollapse・same-orientation・perpendicular時の実`atan2`到達classを共有残差で全拒否できる場合だけ肯定する。初期座標、solver収束、epsilon、近似長、別点・別辺を証拠にしない。legacy 8 variantを診断wireから削除したとは主張せず、`DirectConflict`へ昇格しない互換variantとして隔離する。全11種の一般充足可能性、完全な一般矛盾原因、一般最小不能部分集合は未完成である |

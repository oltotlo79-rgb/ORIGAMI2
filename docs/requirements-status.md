...
2026-07-20再集計: LIN-004、EDT-001/007/008/009/010/012、VAL-002/004/008、SIM-003/006/007/009、INS-001/002/003/004/005/006/007/010、IO-003、OPS-007、PRJ-007/009の完了、およびSIM-008の初期利用者経路接続により、現在の正本87行は **実装済み83 / 部分実装3 / 未着手1**。
...
| SIM-008 | 部分実装 | topology snapshotへ元紙ID付きの決定的material componentを追加し、境界から境界へ達するCutとclosed cut loopを別componentへ分離する。Faceは外周に加えて穴・seamのversion互換表現を持ち、closed loopでは外側faceの時計回り穴と内側pieceを、紙内部の孤立したopen/branched Cutでは両岸を持つzero-area seamを決定的に対応付ける。Cut incidenceは左右faceを保持するがhinge adjacencyへ入れず、frontendも全face/component partitionを再検証して両片を静的3D表示する。切断禁止は従来どおり拒否する。境界へ接続するopen/branched seamのface分解と、穴・seamの3D描画を残す |

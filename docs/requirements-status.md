...
2026-07-26現行再集計: EDT-007はコンパス円の円×線・円×円交点を既存の頂点追加・辺分割・Undo/Redo経路へ接続したため実装済みへ更新した。EDT-009はwire互換の`DirectConstraintConflictKindV1` enum 21 variantを維持するが、実際のbinary64 residualに対するsound allowlistは7 variantだけであり、残る14 variantは`Unknown`へfail-closedとする。16制約以下の最小基数subset探索もこの7種のsoundな直接矛盾oracleだけを使用し、全制約種の一般充足可能性・一般矛盾原因・一般MUSは未完成なので部分実装を維持する。現在の正本87行は **実装済み85 / 部分実装2 / 未着手0**。

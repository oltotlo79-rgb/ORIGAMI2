...
2026-07-20再集計: LIN-004、EDT-001/007/008/009/010/012、VAL-002/004/008、SIM-003/006/007/008/009、INS-001/002/003/004/005/006/007/010、IO-002/003、OPS-007、PRJ-007/009の完了により、現在の正本87行は **実装済み85 / 部分実装1 / 未着手1**。
...
| INS-007 | 実装済み | 3D panelの明示toggleをONにした後、手動で完了した3D姿勢変更を完全hinge vector・固定面付きの通常手順として自動追加する。ON直後の既存姿勢、timeline再生、running姿勢、stale binding、同一操作の重複は記録しない。自動手順は通常timelineへ入り、題名・説明・注意・時間・camera/矢印/注目箇所・姿勢・順序を既存UIで再編集でき、Undo/Redoと`.ori2`保存の対象になる |
...
| IO-008 | 部分実装 | OBJ/GLBをBlender・Web・他3Dアプリへ、STLを3Dプリンター向けworkflowへ受け渡せる静的出力を実装。GLBはPBR materialを内包し、紙厚正では表裏capと境界wallを持つclosed solidへ正規化RGBA頂点色で表裏色を反映する。生成3形式は`gltf`・`tobj`・`stl_io`の独立readerでも自動受入する。同一topologyの2〜256 frameと厳密増加timeをSTEP morph-target animationへ変換する64 MiB上限の形式基盤に加え、認証済みinstruction timelineをplanar初期frame＋各native poseへsolveしてnative immutable GLB stageにし、dialog前後・書込直前の再認証、no-follow原子的保存、cancel retryを行うnative command、strict frontend client、日英preview/save/cancel/retry UIまで接続した。PNG/JPEG埋込texture、UV、PBR baseColorTextureも16 MiB上限で形式基盤へ実装し、native pickerからbounded project asset registryへ表裏を登録・選択してrevision固定で解決する。厚さ0は表裏、正厚は表cap・裏cap・side wallを別primitive・materialへ完全分類し、表裏textureをlosslessに出力する。binary64 exact一致する共面隣接faceと、exact capabilityが認証した限定2面1hingeは内部wallを除いてwatertight外殻へ統合する。一般多面・多hinge union、GUIアプリ実機受入と印刷可能性保証を残す |

...
2026-07-23監査再集計: EDT-007はコンパス円の円×線・円×円交点を既存の頂点追加・辺分割・Undo/Redo経路へ接続したため実装済みへ更新した。EDT-009は証明可能な7種の直接矛盾についてcanonicalかつ削除最小な原因制約IDを表示し、それ以外を判定保留にするsoundな境界がMUST文言を満たすため実装済みへ更新した。局所solverの非収束、rank不足、処理上限を矛盾証明へ流用せず、一般MUSを過大主張しない。現在の正本87行は **実装済み86 / 部分実装1 / 未着手0**。
...
| EDT-009 | 実装済み | 7種の証明可能な直接矛盾をproject instance・project・revisionへ束縛して診断し、canonical・重複なし・2件または3件の削除最小な原因制約IDをblocking表示する。直接証明できない制約種、局所solverの非収束・rank不足・処理上限は判定保留とし、矛盾や原因を推測しない |

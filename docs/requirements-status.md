...
2026-07-26現行再集計: EDT-007はコンパス円の円×線・円×円交点を既存の頂点追加・辺分割・Undo/Redo経路へ接続したため実装済みへ更新した。EDT-009は`DirectConstraintConflictKindV1`の19直接矛盾variant（15 fixed-pattern + 4 general-graph）だけを特定し、16制約以下ではsoundな直接矛盾オラクルによる最小基数部分集合も表示するが、全制約種の一般矛盾原因は特定しないため部分実装を維持する。現在の正本87行は **実装済み85 / 部分実装2 / 未着手0**。

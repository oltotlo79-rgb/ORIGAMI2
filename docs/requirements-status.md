...
`prove_static_collision_geometry`へblocking専用で接続した。旧zero-thickness解析の
全canonical pair走査と件数照合を完了した後、bit-exactな紙厚`+0.0`の複数面姿勢だけで
同じissuer/poseに束縛したCayley facadeを実行する。両zero-width gateが横断を証明した
pairがある場合は`ProvenTransversalPenetration`を返し、肯定0件または証拠不足は
`PairEvidenceUnavailable`、資源超過と姿勢不整合は既存のblocking errorへ閉じる。
旧classifierの`Penetrating`、画面側SAT、測定envelope単独の値からこの肯定errorを
生成しない。安全証明のconstructorは従来どおり単一面・zero-pair枝の一箇所だけであり、
複数面の安全集合は拡張していない。

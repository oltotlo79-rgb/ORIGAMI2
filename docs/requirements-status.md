...
2026-07-26現行再集計: EDT-007はコンパス円の円×線・円×円交点を既存の頂点追加・辺分割・Undo/Redo経路へ接続したため実装済みへ更新した。EDT-009はlegacy 21 variantのwire互換を維持し、10/11制約種を横断する有界binary64 exact-zero closure専用tagを2件追加して`DirectConstraintConflictKindV1`を合計23 variantとした。sound familyは12 variant、残るlegacy 11 variantは`Unknown`へfail-closedとする。16制約以下のsubset探索はこの12種のsound oracleだけを使用し、結果をsemantic MUSではなくoracleが認識した最小基数proof coreとして扱う。全11種の一般充足可能性、完全な一般矛盾原因、semantic MUSは未完成なので部分実装を維持する。現在の正本87行は **実装済み85 / 部分実装2 / 未着手0**。
...
| EDT-009 | 部分実装 | `DirectConstraintConflictKindV1`のlegacy 21 variantをwire互換として維持し、10/11制約種を横断する有界binary64 exact-zero closure専用tagを2件追加して合計23 variantとした。実残差で`DirectConflict`へ昇格するsound allowlistは`DifferentFixedLengths`、正の非退化providerを同一edgeへ持つ`HorizontalAndVertical`、`EqualLengthWithDifferentFixedLengths`、consistentな正の有限固定長2件と正の有限比率をsolverと同じ演算順で評価した残差が非ゼロまたは非有限となる`LengthRatioWithIncompatibleFixedLengths`、同じordered edge pairの異なる正有限比率2件とconsistentな正有限固定分母から得るbinary64積が異なるか非有限となる場合だけ3-IDで肯定する`DifferentLengthRatios`、`DifferentFixedLengthsInEqualLengthComponent`、direct `ParallelWithPerpendicularOrientations`、`Horizontal`/`Vertical`のendpoint座標等価pathでforced-zero edgeを作り`EqualLength`双方向と`LengthRatio` denominator-zero→numerator-zeroだけを伝播して正有限固定長へ到達する`PositiveFixedLengthInBoundedZeroLengthClosure`、同じclosureがproductionのbinary64残差でcollapseを必ず拒否できる`PointOnLine`のline、`MirrorSymmetry`のaxis、`AngleBisector`の3辺、`Parallel`の2辺、`FixedAngle`の2辺へ到達する`ZeroLengthClosureReachesNondegenerateProvider`に加え、consistentな正有限固定長からsolverと同じbinary64演算順で導出した共有残差が非ゼロまたは非有限となる`EqualLengthWithNonUnitRatioAndFixedLength`、`NonReciprocalLengthRatiosWithFixedLength`、`NonUnitLengthRatioCycleWithFixedLength`の合計12種である。残る11 variantはcollapse、signed-zero、角度wrap、丸め、正規化residualの実反例があるため一般solverの証明なしに肯定せず、関係する制約IDを保持して`Unknown`へfail-closedとする。16制約以下ではこの12種だけをsoundなsubset oracleとして最大65,535回探索し、canonicalな最小基数proof coreを表示するが、各1件削除後の独立SAT certificateを伴うsemantic MUSとは扱わない。16制約超では肯定済み直接矛盾を維持したままsubset最小化未実行を明示する。zero closure自体は256制約までを独立のwork・storage上限下で調べ、制約数・work・storage・取消・deadlineの原因を区別して`Unknown`へ閉じる。desktop取消はproject instance・project・revision・request generationの完全一致時だけactive tokenへ伝達し、2秒のabsolute deadlineを持つ。`HorizontalAndVertical`の第三原因は同一edgeのcollapseを実solver residualで拒否する正の非退化providerでなければならず、各ratio familyはexact-real積、逆向き除算、epsilon、初期座標を根拠にせず、solver共有のbinary64残差がゼロなら`Unknown`へ閉じる。`DifferentLengthRatios`も2つの積が同じ有限binary64値へ丸まる場合は共有underflow zeroを含め`Unknown`へ閉じ、固定分母を欠く旧2-ID原因を生成しない。zero closureのratio逆向きも正subnormal denominatorが正の比率乗算で+0へunderflowするSAT反例があるため使用しない。`Parallel`はzero伝播に使わずterminalに限定し、`FixedAngle`はcollapse時の実`atan2`候補0/πを共有残差で両方拒否できる場合だけ肯定する。初期座標、solver収束、epsilon、近似長、別点・別辺を証拠にしない。legacy 11 variantを診断wireから削除したとは主張せず、`DirectConflict`へ昇格しない互換variantとして隔離する。全11種の一般充足可能性、完全な一般矛盾原因、一般最小不能部分集合は未完成である |

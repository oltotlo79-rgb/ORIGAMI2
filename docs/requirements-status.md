...
2026-07-23監査再集計: EDT-007はコンパス円を目視表示するが円交点snapを欠き、EDT-009は7種の直接矛盾だけで一般矛盾原因を特定しないため部分実装へ是正した。現在の正本87行は **実装済み84 / 部分実装3 / 未着手0**。
...
| EDT-007 | 部分実装 | 選択頂点を中心に表示単位で半径を指定するコンパス円を最大64個まで表示・消去できる。円×線・円×円の交点snapは未実装であり、現状は目視用guideに限定する |
...
| EDT-009 | 部分実装 | 7種の直接矛盾をproject instance・project・revisionへ束縛して診断し、canonicalな原因制約IDをblocking表示する。完全solverが必要な一般矛盾は判定保留であり、最小不能部分集合の特定を残す |

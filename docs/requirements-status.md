...
2026-07-26現行再集計: EDT-007はコンパス円の円×線・円×円交点を既存の頂点追加・辺分割・Undo/Redo経路へ接続したため実装済みへ更新した。EDT-009はwire互換の`DirectConstraintConflictKindV1` enum 21 variantを維持するが、実際のbinary64 residualに対するsound allowlistは5 variantだけであり、残る16 variantは`Unknown`へfail-closedとする。16制約以下の最小基数subset探索もこの5種のsoundな直接矛盾oracleだけを使用し、全制約種の一般充足可能性・一般矛盾原因・一般MUSは未完成なので部分実装を維持する。現在の正本87行は **実装済み85 / 部分実装2 / 未着手0**。
...
| EDT-009 | 部分実装 | `DirectConstraintConflictKindV1`のenum 21 variantはwire互換として維持するが、実際のbinary64 residualで`DirectConflict`へ昇格するsound allowlistは`DifferentFixedLengths`、正の非退化providerを同一edgeへ持つ`HorizontalAndVertical`、`EqualLengthWithDifferentFixedLengths`、`DifferentFixedLengthsInEqualLengthComponent`、direct `ParallelWithPerpendicularOrientations`の5種だけである。残る16 variantはcollapse、signed-zero、角度wrap、丸め、正規化residualの実反例があるため一般solverの証明なしに肯定せず、関係する制約IDを保持して`Unknown`へfail-closedとする。16制約以下ではこの5種だけをsoundなsubset oracleとして最大65,535回の探索で最小基数原因を表示し、16制約超では肯定済み直接矛盾を維持したまま最小化未実行を明示する。`HorizontalAndVertical`の第三原因は同一edgeのcollapseを実際のsolver residualで拒否する正の非退化providerでなければならず、初期座標、epsilon、近似長、別点・別辺を証拠にしない。enumの他16種を診断wireから削除したとは主張せず、`DirectConflict`へ昇格しない互換variantとして隔離する。全制約種の一般充足可能性、一般矛盾原因、一般最小不能部分集合は未完成である |

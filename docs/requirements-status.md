...
現在の行単位集計は **実装済み25 / 部分実装31 / 未着手30**。第三者監査本文の集計値「21 / 23 / 42」は、同レポート内の各行判定を合計した値と1件ずれるため、以下の行単位判定を正本とする。
...
| IO-006 | 部分実装 | 現在の一枚紙展開図をFOLD 1.2または静的SVGへ、情報損失確認、revision固定stage、native原子的保存付きで書き出せる。正確な展開図PDF/DXFを残す |
| IO-007 | 未着手 | OBJ/STL/glTF exporterなし |
| IO-008 | 未着手 | 外部3D用途向け出力workflowなし |
| IO-009 | 部分実装 | FOLD/SVG書き出し前に、紙の見た目、ID・履歴、3D表示、折り手順、切断許可など形式が保持しない情報を表示し、明示確認をnativeでも強制する。将来のPDF/DXF/3D exporterへの共通適用を残す |

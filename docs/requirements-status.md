...
現在の行単位集計は **実装済み23 / 部分実装30 / 未着手33**。第三者監査本文の集計値「21 / 23 / 42」は、同レポート内の各行判定を合計した値と1件ずれるため、以下の行単位判定を正本とする。
...
| IO-004 | 部分実装 | FOLD 1/1.1/1.2の2D `creasePattern`を、縮尺・B/M/V/C/F/U/J割当・情報損失警告の確認画面を介して新規未保存projectへ取り込める。SVG読込、FOLDの複数frame・3D `foldedForm`・穴/複数紙を残す |

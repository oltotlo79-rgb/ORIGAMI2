...
| SIM-008 | 部分実装 | topology snapshotへ元紙ID付きの決定的material componentを追加し、境界から境界へ達するCutとclosed cut loopを別componentへ分離する。Faceは外周に加えて穴・seamのversion互換表現を持ち、closed loopでは外側faceの時計回り穴と内側pieceを決定的に対応付ける。Cut incidenceは左右faceを保持するがhinge adjacencyへ入れず、frontendも全face/component partitionを再検証して両片を静的3D表示する。切断禁止は従来どおり拒否する。開いたseamと分岐Cutのface grouping・描画を残す |

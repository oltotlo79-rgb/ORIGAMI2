...
2026-07-23監査再集計: EDT-007はコンパス円の円×線・円×円交点を既存の頂点追加・辺分割・Undo/Redo経路へ接続したため実装済みへ更新した。EDT-009は`DirectConstraintConflictKindV1`の18直接矛盾variant（14 pairwise + 4 general-graph）だけを特定し、16制約以下ではsoundな直接矛盾オラクルによる最小基数部分集合も表示するが、全制約種の一般矛盾原因は特定しないため部分実装を維持する。現在の正本87行は **実装済み85 / 部分実装2 / 未着手0**。
...
| EDT-009 | 部分実装 | `DirectConstraintConflictKindV1`の18直接矛盾variant（14 pairwise + 4 general-graph）をproject instance・project・revisionへ束縛して診断し、canonicalな削除最小原因制約IDをblocking表示する。16制約以下で直接矛盾が成立した場合は、最大65,535回のsoundな直接矛盾subset oracleで最小基数の矛盾部分集合を証明して表示する。16制約超では直接矛盾の肯定を維持したまま最小化だけを未実行と明示し、一般充足可能性の`Unknown`と混同しない。同一辺の水平・垂直はzero-length解を持つため、それを排除する正の固定長を含む3制約の場合だけ直接矛盾とする。正の固定長へ接続した一般長さ比グラフでは決定的spanning forestのexact binary64有理potentialから非整合な単純cycleと固定長までのconnectorを抽出する。一般等長グラフではmulti-source BFSにより、等長pathで接続されたbinary64 bit非同一の正固定長2件を抽出する。一般平行グラフでは、各平行制約の正規化残差が参加辺の非ゼロ長を要求することを根拠に、平行pathで接続された水平・垂直orientation、または同じ平行component内の2辺へ数値的に0度でも180度でもない固定角が指定された矛盾を抽出する。固定角の2辺を結ぶ平行pathは1制約以上を必須とし、最短path間ではcanonicalな原因ID列を選ぶ。一般グラフ原因は最大256 IDとし、長さ比はpotential合計1,048,576 bit・算術work 2,000,000、等長・平行はarcとpath抽出を含むwork各40,000をchecked上限として超過時は判定保留へ閉じる。同じ役割順の`center/source/target`へ異なる回転角を指定した2件は、`{center,source}`または`{center,target}`の実在辺へconsistentな正の固定長がある場合だけ直接矛盾とする。`det(Rot(alpha) - Rot(beta)) = 4 sin²((alpha - beta) / 2)`が非零なので唯一の同時解は全役割の同一点collapseであり、正の半径がそれを排除する。半径証拠がない2件は従来どおり判定保留へ閉じる。完全solverが必要な他の一般矛盾は判定保留であり、全制約種の一般最小不能部分集合の特定を残す |

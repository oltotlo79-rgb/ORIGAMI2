...
2026-07-23監査再集計: EDT-007はコンパス円の円×線・円×円交点を既存の頂点追加・辺分割・Undo/Redo経路へ接続したため実装済みへ更新した。EDT-009は9種のsoundな直接矛盾だけで一般矛盾原因を特定しないため部分実装を維持する。現在の正本87行は **実装済み85 / 部分実装2 / 未着手0**。
...
| EDT-009 | 部分実装 | 9種のsoundな直接矛盾をproject instance・project・revisionへ束縛して診断し、canonicalな削除最小原因制約IDをblocking表示する。追加2種は、等長と非1比率、または非相反な双方向比率について、正の固定長でゼロ長解を排除できる場合だけ3制約の直接矛盾とする。完全solverが必要な一般矛盾は判定保留であり、最小不能部分集合の特定を残す |

...
現在の行単位集計は **実装済み27 / 部分実装30 / 未着手30**。2026-07-18のオーナー決定によりSIM-010を追加し、macOS自動ビルド・CI検証へ確定したOPS-008を実装済みとして再評価した。第三者監査本文の旧集計値は要件変更前の履歴であり、以下の87行の判定を正本とする。
...
| PRJ-006 | 実装済み | 0以上の紙厚を保存し、初版正式仕様の中央面基準近似で表示・衝突へ反映 |
...
| VAL-003 | 未着手 | 時間制限つき全体平坦折り3値判定と、その判定で用いる層順序管理なし |
| VAL-004 | 部分実装 | 選択1ヒンジの線形CCDでは衝突直前停止と理由表示が動作。全3D折り操作経路への適用を残す |
| VAL-005 | 部分実装 | 処理中表示と作業上限による判定不能はある。正式な時間制限との結合を残す |
...
| VAL-009 | 部分実装 | clear/blocked/indeterminateで可・不可・不明を区別。利用者中止と計算エラーの独立したUI状態を残す |
...
| SIM-005 | 実装済み | 表示厚と判定厚を分離し、正式仕様`centered_mid_surface_v1`で衝突へ反映 |
...
| SIM-010 | 未着手 | 現在3D状態の一直線による複数層一括折り、層別山谷線の展開図追加、1 step記録を未実装。VAL-003の層順序管理完成後に着手する |

...
| EDT-009 | 部分実装 | `DirectConstraintConflictKindV1`のlegacy 21 variantをwire互換として維持し、有界binary64 exact-zero closure専用tag 2件と二固定root比率domain専用tag `InconsistentLengthRatioGraphBetweenFixedLengths`を加えた合計24 variantである。実残差で`DirectConflict`へ昇格し、各原因削除後の独立SAT certificateまで再認証するsound semantic familyは`DifferentFixedLengths`、`DifferentFixedAngles`、`HorizontalAndVertical`、`EqualLengthWithDifferentFixedLengths`、`LengthRatioWithIncompatibleFixedLengths`、`DifferentLengthRatios`、`DifferentFixedLengthsInEqualLengthComponent`、`PerpendicularOrientationsInParallelComponent`、`ParallelWithPerpendicularOrientations`、`SameOrientationWithFixedNonParallelAngle`、`PerpendicularOrientationsWithFixedNonRightAngle`、`DifferentRotationalSymmetryAnglesWithFixedRadius`、`NonComplementaryInverseRotationalSymmetryAnglesWithFixedRadius`、`RotationalSymmetryWithCollinearRadius`、`MirrorSymmetryWithPointOnAxisAndFixedSeparation`、`PositiveFixedLengthInBoundedZeroLengthClosure`、`ZeroLengthClosureReachesNondegenerateProvider`、`EqualLengthWithNonUnitRatioAndFixedLength`、`NonReciprocalLengthRatiosWithFixedLength`、`NonUnitLengthRatioCycleWithFixedLength`、`InconsistentLengthRatioGraphWithFixedLength`、`InconsistentLengthRatioGraphBetweenFixedLengths`の合計22/24種である。二固定root familyは異なる実在辺のconsistentな正有限固定長を、順方向のproduction乗算と除算を使わない完全な逆向き乗算preimageで同じexact edgeまで伝播し、保守domainが厳密に分離し、原因subgraphのproduction向き再生が有限な場合だけ肯定する。原因は固定長2件と比率2〜254件の最大256 IDであり、丸めalias、非有限再生、resource、取消、deadlineは肯定しない。`RotationalSymmetryWithCollinearRadius`はexact 90/270度の回転、実在する有向center→source辺、同じ辺を指定するtargetの`PointOnLine`という厳密な2-ID原因だけを所有する。`PerpendicularOrientationsInParallelComponent`は、異なる3実在辺`e0`・`e1`・`e2`に対する`Horizontal(e0)`、`Parallel(e0,e1)`、`Parallel(e1,e2)`、`Vertical(e2)`、bit-exactな`FixedLength(e2, 1.0)`から成るcanonical 5-IDのunit-terminal two-hop coreだけを所有する。semantic MUSへの昇格は3辺がsource topologyでcommon-center starを成し、5件すべての削除を専用の完全finite production residual-only overlayで独立exact SAT再認証できる場合に限定する。非単位終端、固定長欠落、3-hop以上または任意長のparallel pathはsolver-required `Unknown`を維持し、nonstar topologyはdirect theoremを保持しても専用semantic constructorでは肯定しない。残る2 family、`NonParallelFixedAngleInParallelComponent`と`ParallelWithFixedNonParallelAngle`は、正規化残差の非推移性、collapse、signed-zero、角度wrap、丸め、または削除後もUNSATとなる冗長原因の反例があるため、一般solverの証明なしに`DirectConflict`やsemantic MUSへ誤昇格しない。これらはcanonicalな関係制約IDを保持してblockingな`Unknown`へfail-closedとし、project mutationを認可しない。16制約以下ではこの22種だけをsoundなsubset oracleとして最大65,535回探索し、各1件削除後の独立SAT certificateを再認証できたcanonical最小coreだけを`geometric_constraint_deterministic_binary64_semantic_mus_v3`のsemantic MUSとして扱う。zero closureと一般比率domainは256制約までを独立のwork・storage上限下で調べる。zero closureではratio逆向きを使用せず、一般比率domainの逆向きは除算せず、underflow plateauとoverflow境界を保持する非負有限binary64乗算preimageだけを使用する。ratio familyはexact-real積、epsilon、初期座標を根拠にしない。`Parallel`はzero伝播に使わずterminalに限定し、`FixedAngle`は実`atan2`到達classを共有残差で全拒否できる場合だけ肯定する。残る2 familyを診断wireから削除したとは主張せず、肯定権威を持たない互換variantとして隔離する。全11種の一般充足可能性、完全な一般矛盾原因、一般最小不能部分集合は未完成である |

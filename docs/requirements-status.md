...
| IO-008 | 部分実装 | OBJ/GLBをBlender・Web・他3Dアプリへ、STLを3Dプリンター向けworkflowへ受け渡せる静的出力を実装。GLBはPBR materialを内包し、紙厚正では表裏capと境界wallを持つ面別closed solidへ正規化RGBA頂点色で表裏色を反映する。生成3形式は`gltf`・`tobj`・`stl_io`の独立readerでも自動受入する。同一topologyの2〜256 frameと厳密増加timeをSTEP morph-target animationへ変換する64 MiB上限の形式基盤に加え、認証済みinstruction timelineをplanar初期frame＋各native poseへsolveしてnative immutable GLB stageにするpreview/cancel commandを追加した。PNG/JPEG埋込texture、UV、PBR baseColorTextureも16 MiB上限で形式基盤へ実装し独立readerで検証する。animationの保存・frontend UI、project texture assetとの接続、面間union、GUIアプリ実機受入と印刷可能性保証を残す |

...
| IO-008 | 部分実装 | OBJ/GLBをBlender・Web・他3Dアプリへ、STLを3Dプリンター向けworkflowへ受け渡せる静的出力を実装。GLBはPBR materialを内包し、紙厚正では表裏capと境界wallを持つ面別closed solidへ正規化RGBA頂点色で表裏色を反映する。折り手順を時間軸へ変換したanimation、texture、面間unionと印刷可能性保証を残す |

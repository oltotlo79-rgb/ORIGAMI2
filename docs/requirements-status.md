...
| LIN-004 | 部分実装 | version固定のordered layer文書、implicit default layer、折り線edge assignment、作成・改名・並べ替え・削除・割当command、全geometry分割時の継承、Undo/Redo、`.ori2` required feature、通常open・復旧・strict IPC snapshotに加え、作成・改名・並べ替え・削除・選択折り線の割当を行う日英layer管理UIを実装。注釈・下絵objectの編集と表示・lock・透明度を残す |

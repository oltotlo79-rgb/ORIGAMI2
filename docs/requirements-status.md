...
| EDT-005 | 部分実装 | 新規作成と既存長方形用紙resizeの幅・高さについて、表示単位込みの原式とnative採用値をversion固定schemaで保持し、native再評価・bit一致検証後に原子的変更する。式bindingのUndo/Redo両stackを全編集履歴と同期し、履歴上限、`.ori2`、展開folder、復旧保存・再読込、式/評価値切替表示まで接続。頂点座標・長さ角度作図など用紙寸法以外の式binding永続化を残す |

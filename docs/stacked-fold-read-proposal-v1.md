...
desktopの`propose_current_stacked_fold_read`は、strictなproject/revisionと一直線候補を受け、model ID、generation binding、SHA-256 cell key、完全なbottom-to-top face列、target face和集合、有限work countだけをbounded DTOへ写す。process-wide native pose worker permitを解析開始前に取得し、同時実行は変更なしのbusyへ閉じる。permitはblocking workerのcancel中も終了まで保持し、live authority再検証後にだけ解放する。応答は`authorizesProjectMutation=false`と`authorizesApplyStackedFold=false`を固定し、opaque guard、proposal、model、pose、layer snapshotをWebViewへ渡さない。このcommandはUIへ未接続であり、単独ではSIM-010の利用者経路を構成しない。
...
さらに証明済みtarget topologyを再構築し、native `MaterialTreeKinematicsModel`へ有限上限つきでadmitする。treeで表現できないface隣接はこの段階で失敗し、応答にはtarget material face数とhinge数だけを追加する。admit後はcurrent source poseのissuerを再検証し、既存hinge subdivisionへsource角度、新規crease subdivisionへ0度を割り当て、source fixed faceのcanonical descendantを固定してtarget初期poseをsolveする。no-hinge sourceでは最初のcanonical descendantを固定する。全descendant transformがsource face transformと完全一致する場合だけ返す。続いて証明済みの全新規crease subdivisionだけを同一要求角へ置換したendpoint poseをnative modelでsolveする。要求角は有限な`0 < angle <= 180`だけを受理し、期待creaseがtarget hingeから欠落すれば失敗する。このendpoint poseはcontinuous collisionまたはsafe-stopをまだ証明せず、commit authorityではない。既存hingeを分割してdual graphがcycleになる一般targetは現行tree modelの対象外として失敗する。

要求角endpointは同じnative model/poseとpaper thicknessを公開静的衝突診断へ渡し、全unordered face pairのseparated/touching/allowed/penetrating/indeterminate件数とblocking holdをDTOへ写す。これはendpoint一姿勢の診断であり、開始角からendpointまでの経路安全性を意味しない。blocking holdを安全成功へ変換せず、mutation authorityも発行しない。

要求角がbit-exact 180度の場合だけ、target immutable geometryからtopologyとlocal necessary conditionを再構築し、global flat-foldability solverでtarget layer-order certificateを再証明する。Possibleの場合はmaterial face数とoverlap cell数だけをDTOへ写し、certificate本体はWebViewへ渡さない。Impossible/Unknownは`certified=false`、180度以外は`applicable=false`とし、いずれも層順authorityへ昇格させない。commit時には同じtarget geometry・pose・live bindingへ再結合する必要がある。

...
| 高精度数値 | num-bigint + num-rational（証明計算） | 保存済みbinary64をexact有理数へ変換する。将来の数式入力用高精度浮動小数は別途選定 |
...
`.ori2`の開く・保存・別名保存はRust側の専用コマンドとネイティブダイアログだけで行う。WebViewはパスを直接読書きせず、汎用ファイルシステム権限も持たない。これにより、UIへ渡す権限をウィンドウイベントの待受けと破棄など必要最小限に限定する。

保存は同一保存先に対する一時書込み、ファイル同期、生成した`.ori2`コンテナの再検証、原子的置換の順で行い、途中失敗時は既存ファイルを維持する。ネイティブダイアログ中に編集状態が変化する可能性があるため、操作開始時のproject IDとrevisionを確保し、適用直前に再照合する。UI側の操作直列化とRust側の照合を併用し、古い非同期結果で現在のプロジェクトを上書きしない。

未保存状態での通常のウィンドウ終了、およびmacOSのCmd+Q／アプリメニュー終了は確認ガードへ通す。ただし、使用中のTauriランタイムではmacOSのDockメニュー終了やOS終了要求が同じイベント経路を通らない可能性がある。macOS配布完了前に実機で検証し、自動復旧保存またはネイティブdelegateによる追加保護を実装する。
...
Rust側の全体平坦折り判定と厳密幾何検証にはpure Rustの`num-bigint`と`num-rational`を採用した。保存済みbinary64を値の変更なしにexact有理数へ変換し、整数bit長、exact演算回数、証明関連storageを上限化する。この選定はEDT-004/005の数式入力・式保持用backendを確定するものではない。

Rugは任意精度整数・有理数・浮動小数を提供するが、GMP/MPFR/MPC依存とクロスビルド負担があるため、将来の数式入力用候補として正式採用前にWindows/macOS CIで検証する。

- Rug: <https://docs.rs/crate/rug/latest>

### 2.8 全体平坦折り制約solver

初版は外部SAT/SMT solverへ依存せず、`convex_faces_facewise_v1`専用の決定論的solverをRustで実装した。exact平面反射と重なりcellからcanonical面対の二値変数を作り、Mountain/Valley、推移、taco-taco、taco-tortilla等の許容tupleをBFSで伝播する。未確定成分は明示的な`SearchFrame` stackとassignment trailを用いるDFSで探索し、再帰stackへ依存しない。

可はimmutable geometryから独立に再検証したcertificateがある場合、不可は局所必要条件違反、exact配置矛盾、constraint矛盾または有限全探索の完了で証明できた場合だけ返す。対象外、deadline、cancel、資源上限、証明不足は不可へ変換しない。場所別overlap cell stackを`facewise_layer_order_v1`としてnativeへ保持し、後続SIM-010の正本にする。対象クラス、provenance、上限、UI契約は[全体平坦折り判定と層順序管理の設計](global-flat-foldability-design.md)に固定する。
...
10. 一般経路探索、非凸面、切断・穴を含む次期全体平坦折りmodelで保証可能な対象クラス。初版の凸面facewise 3値判定は選定・実装済み。
...
| num-bigint + num-rational | 全体平坦折り・Rust厳密幾何検証へ採用 |
| rug | 将来の数式入力用比較候補、クロスビルドPoC後確定 |
| 全体平坦折り制約solver | 専用のBFS伝播 + iterative DFS + certificate再検証を採用 |
| 全体平坦折りアルゴリズム | 凸material face対象の`convex_faces_facewise_v1`を採用 |
| 一般折り経路探索 | 未選定。初版の全体平坦折り3値判定とは分離 |
